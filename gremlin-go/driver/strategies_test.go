...
func TestStrategy(t *testing.T) {
	...
	t.Run("Test Bytecode generation for DetachStrategy", func(t *testing.T) {
		g := getModernGraph(t, testNoAuthUrl, &AuthInfo{}, &tls.Config{})
		defer g.remoteConnection.Close()

		config := DetachStrategyConfig{DetachMode: DetachMode.Custom, Keys: []string{"foo", "bar"}}
		bytecode := g.WithStrategies(DetachStrategy(config)).bytecode
		assert.Equal(t, 1, len(bytecode.sourceInstructions))
		assert.Equal(t, 1, len(bytecode.sourceInstructions[0].arguments))
		assert.Equal(t, "withStrategies", bytecode.sourceInstructions[0].operator)
		assert.Equal(t, "org.apache.tinkerpop.gremlin.process.traversal.strategy.finalization.DetachStrategy",
			bytecode.sourceInstructions[0].arguments[0].(*traversalStrategy).name)
		assert.Equal(t, map[string]interface{}{"detachMode": "CUSTOM", "keys": []string{"foo", "bar"}},
			bytecode.sourceInstructions[0].arguments[0].(*traversalStrategy).configuration)
	})

	t.Run("Test DetachStrategy and Vertex with custom fields list", func(t *testing.T) {
		g := getModernGraph(t, testNoAuthUrl, &AuthInfo{}, &tls.Config{})
		defer g.remoteConnection.Close()

		config := DetachStrategyConfig{DetachMode: DetachMode.Custom, Keys: []string{"age"}}
		// vertex contains 2 properties, name and age. I would filter by name, but get only age
		r, err := g.WithStrategies(DetachStrategy(config)).V().Has("person", "name", "marko").Next()
		assert.Nil(t, err)
		assert.NotNil(t, r)
	...
	t.Run("Test DetachStrategy and Vertex with all fields", func(t *testing.T) {
		g := getModernGraph(t, testNoAuthUrl, &AuthInfo{}, &tls.Config{})
		defer g.remoteConnection.Close()

		config := DetachStrategyConfig{DetachMode: DetachMode.All}
		r, err := g.WithStrategies(DetachStrategy(config)).V().Has("person", "name", "marko").Next()
		assert.Nil(t, err)
		assert.NotNil(t, r)
	...
	t.Run("Test DetachStrategy and Vertex with no fields", func(t *testing.T) {
		g := getModernGraph(t, testNoAuthUrl, &AuthInfo{}, &tls.Config{})
		defer g.remoteConnection.Close()

		config := DetachStrategyConfig{DetachMode: DetachMode.None}
		r, err := g.WithStrategies(DetachStrategy(config)).V().Has("person", "name", "marko").Next()
		assert.Nil(t, err)
		assert.NotNil(t, r)
	...
	t.Run("Test DetachStrategy and Edge with custom fields list", func(t *testing.T) {
		g := getModernGraph(t, testNoAuthUrl, &AuthInfo{}, &tls.Config{})
		defer g.remoteConnection.Close()
		...
		properties, ok := edge.Properties.([]interface{})
		assert.True(t, ok)
		assert.Equal(t, 1, len(properties))

		property, ok := properties[0].(*Property)
		assert.True(t, ok)
		assert.NotNil(t, property)
		assert.Equal(t, "weight", property.Key)
		assert.Equal(t, 0.5, property.Value)
	})
