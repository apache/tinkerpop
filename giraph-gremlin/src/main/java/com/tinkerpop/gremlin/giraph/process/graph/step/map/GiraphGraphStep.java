...
public class GiraphGraphStep<E extends Element> extends GraphStep<E> {
    ...
    public void generateTraverserIterator(final boolean trackPaths) {
        ...
        try {
            if (trackPaths)
                this.starts.add(new TraverserIterator(this, Vertex.class.isAssignableFrom(this.returnClass) ? new VertexIterator(this.graph) : new EdgeIterator(this.graph)));
            else
                this.starts.add(new TraverserIterator(Vertex.class.isAssignableFrom(this.returnClass) ? new VertexIterator(this.graph) : new EdgeIterator(this.graph)));
        } catch (Exception e) {
