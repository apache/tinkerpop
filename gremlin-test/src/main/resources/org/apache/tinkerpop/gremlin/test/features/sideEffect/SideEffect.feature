...
Feature: Step - sideEffect()
  ...
  Scenario: g_withSideEffectXx_setX_V_both_both_sideEffectXstoreXxX_byXnameXX_capXxX_unfold
    Given the modern graph
    And the traversal of
      """
      g.withSideEffect("x",{}).V().both().both().sideEffect(__.store("x").by("name")).cap("x").unfold()
      """
    When iterated to list
    Then the result should be unordered
      | result |
