...
Feature: Step - aggregate()
  ...
  Scenario: g_withSideEffectXa_1_sumLongX_V_aggregateXaX_byXageX_capXaX
    ...
    # This graph is the same as ModernGraph, but ages are stored as Long.
      """
      g.addV("person").property("name", "marko").property("age", 29L).as("marko").
        addV("person").property("name", "vadas").property("age", 27L).as("vadas").
        ...
        addV("person").property("name", "peter").property("age", 35L).as('peter').
    ...
    And the traversal of
    """
    g.withSideEffect("a", 1L, Operator.sumLong).V().aggregate("a").by("age").cap("a")
    """
    When iterated to list
    Then the result should be unordered
      | result   |
  ...
  Scenario: g_withSideEffectXa_1_sumLongX_V_aggregateXlocal_aX_byXageX_capXaX
    ...
    And the traversal of
    """
    g.withSideEffect("a", 1L, Operator.sumLong).V().aggregate(Scope.local, "a").by("age").cap("a")
    """
    When iterated to list
    Then the result should be unordered
      | result   |
  ...
  Scenario: g_withSideEffectXa_l1_addAllX_V_aggregateXaX_byXageX_capXaX
    Given the modern graph
    And using the parameter l1 defined as "l[d[1].i,d[2].i,d[3].i]"
    And the traversal of
    """
    g.withSideEffect("a", l1, Operator.addAll).V().aggregate("a").by("age").cap("a")
    """
    When iterated next
    Then the result should be unordered
    | result |
  ...
  Scenario: g_withSideEffectXa_l1_addAllX_V_aggregateXlocal_aX_byXageX_capXaX
    Given the modern graph
    And using the parameter l1 defined as "l[d[1].i,d[2].i,d[3].i]"
    And the traversal of
    """
    g.withSideEffect("a", l1, Operator.addAll).V().aggregate(Scope.local, "a").by("age").cap("a")
    """
    When iterated next
    Then the result should be unordered
      | result |
  ...
  @GraphComputerVerificationInjectionNotSupported
  Scenario: g_withSideEffectXa_l1_assignX_V_aggregateXaX_byXageX_capXaX
    Given the modern graph
    And using the parameter l1 defined as "l[d[1].i,d[2].i,d[3].i]"
    And the traversal of
    """
    g.withSideEffect("a", l1, Operator.assign).V().aggregate("a").by("age").cap("a")
    """
    When iterated next
    Then the result should be unordered
    | result |
  ...
  @GraphComputerVerificationInjectionNotSupported
  Scenario: g_withSideEffectXa_l1_assignX_V_order_byXageX_aggregateXlocal_aX_byXageX_capXaX
    Given the modern graph
    And using the parameter l1 defined as "l[d[1].i,d[2].i,d[3].i]"
    And the traversal of
    # add order().by("age") to deterministically assign a vertex with the largest age value at the end
    """
    g.withSideEffect("a", l1, Operator.assign).V().order().by("age").aggregate(Scope.local, "a").by("age").cap("a")
    """
    When iterated next
    Then the result should be unordered
      | result |
