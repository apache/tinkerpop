...
Feature: Step - range()
  ...
  Scenario: g_V_out_whereXhasXnameX_order_limitX1XX_out_whereXhasXnameX_order_limitX1XX
    Given the modern graph
    And the traversal of
      """
      g.V().out().where(has("name").order().limit(1)).out().where(has("name").order().limit(1))
      """
    When iterated to list
    Then the result should be unordered
      | result |
  ...
  Scenario: g_V_hasXnameXJAMXX_repeatXoutXfollowedByX_order_limitX2XX_timesX2X
    Given the grateful graph
    And the traversal of
      """
      g.V().has('name','JAM').repeat(out('followedBy').order().limit(2)).times(2)
      """
    When iterated to list
    Then the result should have a count of 2

  Scenario: g_V_hasXnameXJAMXX_outXfollowedByX_order_limitX2X_outXfollowedByX_order_limitX2X
    Given the grateful graph
    And the traversal of
      """
      g.V().has('name','JAM').out('followedBy').order().limit(2).out('followedBy').order().limit(2)
      """
    When iterated to list
    Then the result should have a count of 2

  Scenario: g_V_hasXnameXDRUMSXX_repeatXinXfollowedByX_order_rangeX1_4XX_timesX2X
    Given the grateful graph
    And the traversal of
      """
      g.V().has('name','DRUMS').repeat(in('followedBy').order().range(1,4)).times(2)
      """
    When iterated to list
    Then the result should have a count of 3

  Scenario: g_V_hasXnameXDRUMSXX_inXfollowedByX_order_rangeX1_4X_inXfollowedByX_order_rangeX1_4X
    Given the grateful graph
    And the traversal of
      """
      g.V().has('name','DRUMS').in('followedBy').order().range(1,4).in('followedBy').order().range(1,4)
      """
    When iterated to list
    Then the result should have a count of 3

  # global limits in choose
  Scenario: g_V_chooseXvaluesXageX_isXlteX30XX_out_order_limitX1X_out_order_limitX2XX
    Given the modern graph
    And the traversal of
      """
      g.V().choose(values('age').is(lte(30)),out().order().limit(1),out().order().limit(2))
      """
    When iterated to list
    Then the result should have a count of 3

  # local limits in choose   
  Scenario: g_V_chooseXvaluesXageX_isXlteX30XX_localXout_order_limitX1XX_localXout_order_limitX2XXX
    Given the modern graph
    And the traversal of
      """
      g.V().choose(values('age').is(lte(30)),local(out().order().limit(1)),local(out().order().limit(2)))
      """
    When iterated to list
    Then the result should have a count of 4

  Scenario: g_V_hasXnameXHEY_BO_DIDDLEYXX_unionXoutXfollowedByX_order_limitX2X_outXsungByX_order_limitX1XX_unionXoutXfollowedByX_order_limitX2X_outXsungByX_order_limitX1XX
    Given the grateful graph
    And the traversal of
      """
      g.V().has('name','HEY BO DIDDLEY').
                union(out('followedBy').order().limit(2),
                      out('sungBy').order().limit(1)).
                union(out('followedBy').order().limit(2),
                      out('sungBy').order().limit(1))
      """
    When iterated to list
  ...
  Scenario: g_V_hasXnameXHEY_BO_DIDDLEYXX_repeatXunionXoutXfollowedByX_order_limitX2X_outXsungByX_order_limitX1XXX_timesX2X
    Given the grateful graph
    And the traversal of
      """
      g.V().has('name','HEY BO DIDDLEY').repeat(union(out('followedBy').order().limit(2),out('sungBy').order().limit(1))).times(2)
      """
    When iterated to list
    Then the result should have a count of 3