...
- **`gremlin-core`, `gremlin-test`, or anything they depend on** ripple across the whole project,
  including the OLAP engines (`hadoop-gremlin`, `spark-gremlin`) and the graph implementations
  (`neo4j-gremlin`, `sparql-gremlin`) → run everything:
...
This branch also carries `neo4j-gremlin` and `sparql-gremlin` (graph implementations) and
`gremlin-archetype` (Maven archetypes for new projects), none of which exist on `master`.
