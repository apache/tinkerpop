 ...
 */
public abstract class GroovyWhereTestImpl {
    ...
    public static class StandardTestImpl extends WhereTest {
        ...
        public Traversal<Vertex, Map<String, Object>> get_g_V_hasXageX_asXaX_out_in_hasXageX_asXbX_select_whereXb_hasXname_markoXX() {
            g.V.has('age').as('a').out.in.has('age').as('b').select().where(g.of().as('b').has('name', 'marko'));
        }

        @Override
        public Traversal<Vertex, Map<String, Object>> get_g_V_hasXageX_asXaX_out_in_hasXageX_asXbX_select_whereXa_outXknowsX_bX() {
            g.V().has('age').as('a').out.in.has('age').as('b').select().where(g.of().as('a').out('knows').as('b'));
        }
    }
    ...
    public static class ComputerTestImpl extends WhereTest {
        ...
        @Override
        public Traversal<Vertex, Map<String, Object>> get_g_V_hasXageX_asXaX_out_in_hasXageX_asXbX_select_whereXb_hasXname_markoXX() {
            // TODO: ComputerTestHelper.compute("g.V.has('age').as('a').out.in.has('age').as('b').select().where(g.of().as('b').has('name', 'marko'))", g);
            g.V.has('age').as('a').out.in.has('age').as('b').select().where(g.of().as('b').has('name', 'marko'));
        }

        @Override
        public Traversal<Vertex, Map<String, Object>> get_g_V_hasXageX_asXaX_out_in_hasXageX_asXbX_select_whereXa_outXknowsX_bX() {
            // TODO: ComputerTestHelper.compute("g.V().has('age').as('a').out.in.has('age').as('b').select().where(g.of().as('a').out('knows').as('b'))", g);
            g.V().has('age').as('a').out.in.has('age').as('b').select().where(g.of().as('a').out('knows').as('b'));
        }
    }
}
