...
describe('GraphBinary.TextPSerializer', () => {

  const type_code =  from([0x28]);
  ...
  const cases = [
    ...
    { des:1, err:/{value_flag} is missing/, fq:1, b:[0x28] },
    { des:1, err:/unexpected {value_flag}/, fq:1, b:[0x28,0x10] },
    { des:1, err:/unexpected {value_flag}/, fq:1, b:[0x28,0x02] },
    { des:1, err:/unexpected {value_flag}/, fq:1, b:[0x28,0x0F] },
    { des:1, err:/unexpected {value_flag}/, fq:1, b:[0x28,0xFF] },
  ];
  ...
  describe('#serialize', () =>
    ...
    .forEach(({ v, fq, b }, i) => it(ser_title({i,v}), () => {
      ...
      if (fq !== undefined) {
        assert.deepEqual( textPSerializer.serialize(v, fq), b );
        return;
      }

      // generic case
      assert.deepEqual( textPSerializer.serialize(v, true),  concat([type_code, value_flag, b]) );
      assert.deepEqual( textPSerializer.serialize(v, false), concat([                       b]) );
    }))
  );
  ...
  describe('#deserialize', () =>
    cases.forEach(({ v, fq, b, av, err }, i) => it(des_title({i,b}), () => {
      ...
      if (err !== undefined) {
        if (fq !== undefined)
          assert.throws(() => textPSerializer.deserialize(b, fq), { message: err });
        else {
          assert.throws(() => textPSerializer.deserialize(concat([type_code, value_flag, b]), true),  { message: err });
          assert.throws(() => textPSerializer.deserialize(concat([                       b]), false), { message: err });
        }
        return;
      }
      ...
      // when fq is under control
      if (fq !== undefined) {
        assert.deepStrictEqual( textPSerializer.deserialize(b, fq), {v,len} );
        return;
      }

      // generic case
      assert.deepStrictEqual( textPSerializer.deserialize(concat([type_code, value_flag, b]), true),  {v,len:len+2} );
      assert.deepStrictEqual( textPSerializer.deserialize(concat([                       b]), false), {v,len:len+0} );
    }))
  );
