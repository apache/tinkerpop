...
public class InputRDDTest {
    ...
    @Test
    public void shouldLoadFromArbitraryRDD() {
        final Configuration configuration = new BaseConfiguration();
        configuration.setProperty("spark.master", "local[4]");
        configuration.setProperty("spark.serializer", "org.apache.spark.serializer.KryoSerializer");
        configuration.setProperty(Graph.GRAPH, HadoopGraph.class.getName());
        configuration.setProperty(Constants.GREMLIN_HADOOP_INPUT_RDD, ExampleInputRDD.class.getCanonicalName());
        ...
        assertEquals(Double.valueOf(10.0d), graph.traversal(GraphTraversalSource.computer(SparkGraphComputer.class)).V().values("age").sum().next());
