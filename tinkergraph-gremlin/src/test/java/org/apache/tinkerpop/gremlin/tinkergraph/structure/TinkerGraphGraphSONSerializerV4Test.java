...
/**
 * Serialization and deserialization tests for GraphSON V4 with the TinkerGraph custom type.
 */
public class TinkerGraphGraphSONSerializerV4Test {

    private final Mapper defaultMapperV4 = GraphSONMapper.build()
    ...
    public void shouldDeserializeWellFormedGraph() throws IOException {
        final TinkerMemoryGraph original = TinkerFactory.createModern();
        final GraphWriter writer = getWriter(defaultMapperV4);
        final GraphReader reader = getReader(defaultMapperV4);

        try (final ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            writer.writeObject(out, original);
            final String json = out.toString();
            final TinkerMemoryGraph read = reader.readObject(new ByteArrayInputStream(json.getBytes()), TinkerMemoryGraph.class);
    ...
    public void shouldFailFastOnScalarVerticesField() throws IOException {
        final String malformed = "{\"@type\":\"g:graph\",\"@value\":{\"vertices\":0}}";
        final GraphReader reader = getReader(defaultMapperV4);
        try {
            reader.readObject(new ByteArrayInputStream(malformed.getBytes()), TinkerMemoryGraph.class);
            fail("Expected IOException for malformed g:graph input");
    ...
    public void shouldFailFastOnScalarEdgesField() throws IOException {
        final String malformed = "{\"@type\":\"g:graph\",\"@value\":{\"vertices\":[],\"edges\":0}}";
        final GraphReader reader = getReader(defaultMapperV4);
        try {
            reader.readObject(new ByteArrayInputStream(malformed.getBytes()), TinkerMemoryGraph.class);
            fail("Expected IOException for malformed g:graph input");
    ...
    public void shouldFailFastOnTruncatedInput() throws IOException {
        final String malformed = "{\"@type\":\"g:graph\",\"@value\":{\"vertices\":[";
        final GraphReader reader = getReader(defaultMapperV4);
        try {
            reader.readObject(new ByteArrayInputStream(malformed.getBytes()), TinkerMemoryGraph.class);
            fail("Expected IOException for truncated g:graph input");
        } catch (IOException expected) {
