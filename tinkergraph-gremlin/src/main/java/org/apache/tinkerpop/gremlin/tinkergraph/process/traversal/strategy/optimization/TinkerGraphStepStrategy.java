...
public final class TinkerGraphStepStrategy extends AbstractTraversalStrategy<TraversalStrategy.ProviderOptimizationStrategy> implements TraversalStrategy.ProviderOptimizationStrategy {
    ...
    public void apply(final Traversal.Admin<?, ?> traversal) {
        ...
        for (final GraphStepContract originalGraphStep : TraversalHelper.getStepsOfClass(GraphStepContract.class, traversal)) {
            ...
            while (currentStep instanceof HasStep || currentStep instanceof NoOpBarrierStep) {
                if (currentStep instanceof HasStep) {
                    ...
                    // skip folding this HasStep if any HasContainer holds a child traversal —
                    // its value is dynamic (resolved per-traverser) and cannot be pushed into
                    // TinkerGraphStep. Continue to the next step so that subsequent literal
