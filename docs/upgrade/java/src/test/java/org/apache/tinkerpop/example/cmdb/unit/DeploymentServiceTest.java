...
public class DeploymentServiceTest {
    ...
    @Test
    void timelineDelegatesToGateway() {
        GremlinGateway gateway = mock(GremlinGateway.class);
        List<Map<Object, Object>> expected = List.of(Map.of("event", "incident-linked"));
        when(gateway.fetch(any())).thenReturn((List) expected);

        DeploymentService svc = new DeploymentService(gateway);
        List<Map<Object, Object>> result = svc.timeline("dep-1");
