...
In the OLAP traversal above, the traversal is put into a newly constructed `TraversalVertexProgram` and that program is sent to each vertex in the graph. There are 5 BSP iterations and each iterations is interpreted as such:
 ...
 . Put a traverser on each vertex of the graph.
 . Propagate traversers to all vertices `both`-adjacent.
 . If the vertex doesn't have an `age` property, kill the traversers at that vertex.
 . Propagate all traversers to each vertex's `age` property.
 . Create a `Map` sideEffect which indexes how many times the particular age has been seen.

When the computation is complete a MapReduce job executes which aggregates all the `groupCount` Maps on each vertex and generates a local copy of the Map (thus, turning the distributed Map representation into a local Map representation). The same OLAP traversal can be executed using the standard `g.compute()` model, though at the expense of verbosity.

[source,groovy]
gremlin> result = g.compute().program(TraversalVertexProgram.build().traversal('gremlin-groovy',"TinkerGraph.open().V().both().has('age').value('age').groupCount('a')").create()).submit().get()
==>result[tinkergraph[vertices:6 edges:6],memory[size:3]]
