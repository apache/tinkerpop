...
`MatchStep` was introduced into TinkerPop3 to support a more declarative form of link:http://en.wikipedia.org/wiki/Pattern_matching[pattern matching]. Similar constructs were available in previous TinkerPop versions via the `table()`-step, but that has since been removed in favor of the `match().select()`-pattern. With MatchStep in TinkerPop, a query optimizer exists t
...
| link:http://systap.com[Mike Personick] (2014) | Bigdata representative and OLAP theorist.
