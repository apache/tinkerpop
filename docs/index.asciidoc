...
The BSP model proposed by Pregel and Giraph stores the results of the computation in a distributed manner across the vertices in the graph. However, in many situations, it necessary to aggregate those results into a single result set (i.e. a statistic). For instance, assume a VertexProgram that computes a nominal cluster for each vertex in the graph. At the end of the computation, each vertex will have an hidden property denoting the cluster it is in. In order to answer the following questions, a MapReduce job is required over those resultant properties:

 * How many unique clusters are there?
 * How many vertices are in each cluster?
...
A compressed representation of the `MapReduce` API in TinkerPop3 is provided above. The key idea is that the `map`-stage processes all vertices to emit some key/value pair. Those values are aggregated on their respective key for the `reduce`-stage to do its processing to ultimately yield some iterator of key/value results.
