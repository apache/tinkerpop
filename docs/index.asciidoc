...
NOTE: In Java8, there is no elegant syntax to create a `Map`. Thus, the last traversal above would look as follows in Java8: `g.V.choose({it.get().value('name').length()}, new HashMap() {{ put(5,g.of().in); put(4,g.of().out); }})`
