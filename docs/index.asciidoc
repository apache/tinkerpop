...
CAUTION: Neo4j is licensed as link:http://en.wikipedia.org/wiki/Affero_General_Public_License[AGPL]. Thus, `gremlin-neo4j` (source and binaries) are also licensed as AGPL due to their dependency on the Neo4j libraries. Note that the <<gremlin-console,Gremlin Console>> does not distribute with the Neo4j binaries. To access the binaries, use the `:use` command to download binaries from link:http://search.maven.org/[Maven Central Repository].
...
Neo4j are the creators of their own graph pattern-match query language called link:http://www.neo4j.org/learn/cypher[Cypher]. It is possible to leverage Cypher from within Gremlin by using the `neo4j-gremlin`-specific `Neo4jGraph.cypher()` graph traversal method.
