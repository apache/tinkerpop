...
MatchStep brings functionality similar to link:http://en.wikipedia.org/wiki/SPARQL[SPARQL] to Gremlin. A few of the differentiating qualities include:
...
gremlin> g = TinkerFactory.createClassic()
==>tinkergraph[vertices:6 edges:6]
gremlin> result = g.compute().program(new PageRankVertexProgram()).submit().get()
...
gremlin> g = TinkerFactory.createClassic()
==>tinkergraph[vertices:6 edges:6]
gremlin> result = g.compute().program(new PeerPressureVertexProgram()).mapReduce(new ClusterPopulationMapReduce()).submit().get()
==>result[tinkergraph[vertices:6 edges:6],sideEffects[size:2]]
gremlin> result.sideEffects.get('clusterPopulation')
==>1=5
==>6=1
...
gremlin> g = TinkerFactory.createClassic()
==>tinkergraph[vertices:6 edges:6]
gremlin> result = g.compute().program(new PeerPressureVertexProgram()).
...
==>1=5
==>6=1
gremlin> result.sideEffects.get('clusterCount')
==>2
...
[source,groovy]
gremlin> result = g.compute().program(TraversalVertexProgram.build().traversal(new GremlinGroovySSupplier("TinkerGraph.open().V.both.has('age').age.groupCount.as('a')")).create()).submit().get()
==>result[tinkergraph[vertices:6 edges:6],sideEffects[size:3]]
...
<4> Note that there is a "neo4j" plugin available, but it is not yet "active".
<5> Again, to use the "neo4j" plugin, it must be made "active" with `:plugin use`.
<6> Now when the plugin list is displayed, the "neo4j" plugin is displayed as "active".

NOTE: The Gremlin Console comes configured with the "server" and "utilities" plugins configured as "active" by default.  These plugins and other pre-packaged plugins cannot be removed from the Console as they are a part of the `gremlin-console.jar` itself.  These plugins can only be deactivated.

TIP: It is possible to manage plugin activation and deactivation by manually editing the `ext/plugins.txt` file which contains the classnames of the "active" plugins.  It is also possible to clear dependencies added by `:install` by deleting them from the `ext` directory.
...
[source,groovy]
----
include::../gremlin-server/scripts/generate-classic.groovy[]
----
...
==>Item{resultItem=vadas class=java.lang.String}
==>Item{resultItem=lop class=java.lang.String}
==>Item{resultItem=josh class=java.lang.String}
==>Item{resultItem=ripple class=java.lang.String}
==>Item{resultItem=peter class=java.lang.String}
gremlin> :> g.V.has('name','marko').out('created').name
==>Item{resultItem=lop class=java.lang.String}
gremlin> :> g.E.label.groupCount()
==>Item{resultItem={created=4, knows=2} class=java.lang.String}
----
...
==>Item{resultItem={created=4, knows=2} class=java.util.HashMap}
...
The above JSON represents the "body" of the request to send to Gremlin Server.  There is also a "header".  The "header" basically consists of a "mime type" so that Gremlin Server knows how to deserialize the `RequestMessage`. So, the actual byte array sent to Gremlin Server would be formatted as follows:

image::gremlin-server-request.png[]

The first byte represents the total length of the entire message (excluding this initial byte). The next byte is the length of the "mime type" string value that follows.  Given the default configuration of Gremlin Server, this value should be set to `application/json`.  The "payload" represents the JSON message above encoded as bytes. 
...
|code |A `ResultCode` that is somewhat analagous to an link:http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html[HTTP status code].
|resultType |Helps identify the "type" of value in the `result` - will be an integer value where "0" signifies an "object", "1" signifies a "collection", and "2" signifies an "empty" result.
|result |The return value as classified by the `resultType`.
|========================================================= 

Assuming that Gremlin Server is configured to return one result at a time, the first `ResponseMessage` returned to the client would look something like this:

[source,js]
----
{"result":{"id":2,"label":"vertex","type":"vertex","hiddens":{},"properties":{"name":"vadas","age":27}},
 "code":200,
 "requestId":"1d6d02bd-8e56-421d-9438-3bd6d0079ff1",
 "type":0}
----

As Gremlin Server is streaming results, additional responses will arrive over the WebSocket until the iteration of the result on the server is complete.  The response stream is always marked as "complete" by a terminating message.  The "terminator" will look like this:

[source,js]
{"result":null,"code":299,"requestId":"1d6d02bd-8e56-421d-9438-3bd6d0079ff1","type":0}
...
. Start Gremlin Console and try the `:install` command.  For the case of the Neo4j plugin, it would look like this: `:install com.tinkerpop neo4j-gremlin 3.y.z`.
...
image:gephi-logo.png[width=200] link:http://gephi.github.io/[Gephi] is an interactive visualization and exploration platform for graphs.
...
[source,groovy]
gremlin> :plugin use sugar
==>class com.tinkerpop.gremlin.groovy.loaders.SugarLoader
...
==>sugar activated
gremlin> g.v(1).value('name')  // conventional 
==>marko
...
. Missing element fields: if a vertex or edge's instance field is selected and that field does not exist, then it is assumed the user means `value('fieldName')`.
...
[source,groovy]
----
gremlin> :install com.tinkerpop neo4j-gremlin 3.0.0-SNAPSHOT
...
gremlin> :install com.tinkerpop giraph-gremlin 3.0.0
