...
image::choose-step-example.png[width=700]
...
The BSP model proposed by Pregel stores the results of the computation in a distributed manner as properties of the vertices in the graph. However, in many situations, it necessary to aggregate those resultant properties into a single result set (i.e. a statistic). For instance, assume a VertexProgram that computes a nominal cluster for each vertex. At the end of the computation, each vertex will have an hidden property denoting the cluster it is in. In order to answer the following questions, MapReduce jobs are required:
...
A compressed representation of the `MapReduce` API in TinkerPop3 is provided below. The key idea is that the `map`-stage processes all vertices to emit some key/value pair. Those values are aggregated on their respective key for the `reduce`-stage to do its processing to ultimately yield some iterator of key/value results.
...
gremlin> g = TinkerFactory.createClassic()
==>tinkergraph[vertices:6 edges:6]
gremlin> results = g.compute().program(PeerPressureVertexProgram.create().configuration).
...
`TraversalVertexProgram` supports the execution of `Traversal's in an OLAP manner.
...
The counters that are propagated around the graph are stored in a hidden property called `gremlin.traversalTracker`. When the computation is complete a MapReduce job executes which aggregates all the `gremlin.traversalTracker` counts and displays the results.
...
gremlin> g = TinkerGraph.open()
==>tinkergraph[vertices:0 edges:0]
gremlin> strategy = new PartitionGraphStrategy(Graph.Key.hidden("partition"), "A")
==>PartitionGraphStrategy
...
To the `.bash_profile` file, add the following environmental variables (of course, be sure the directories are the respective locations on the local machine).
 ...
 * `gremlin.outputLocation`: the location to write the final Giraph graph to
 ...
 * `gremlin.traversalSupplierClass`: for `TraversalVertexProgram`, the location of the Gremlin traversal to use
  * This is only required if using GiraphGremlin programmatically versus via the Gremlin Console
 * `gremlin.jarsInDistributedCache `: whether to upload the Giraph-Gremlin jars to the distributed cache's classpath
  * If the jars are already on every node of the cluster, then this is not necessary
...
The above properties file states:
	...
	The location of the graph is tinkerpop-classic-vertices.ldjson. Interpret that file using GraphSONVertexInputFormat. Execute the TraversalVertexProgram on all vertices of the graph using the traversal defined in TraversalSupplier1. Write the computed on GiraphGraph to output/ using the GraphSONOutputFormat. Use 2 workers (thus, 3 map-slots) to execute the job.
...
The properties file in the previous section stated the `gremlin.inputLocation` to be `tinkerpop-classic-vertices.ldjson`. This means that this file will be looked for on link:http://en.wikipedia.org/wiki/Apache_Hadoop#Hadoop_distributed_file_system[HDFS]. Therefore, the file must be put into the filesystem.

[source,bash]
$ hadoop fs -copyFromLocal data/tinkerpop-classic-adjlist.ldjson tinkerpop-classic-vertices.ldjson
$ hadoop fs -ls
Found 1 item
-rw-r--r--   1 marko supergroup       2387 2014-05-26 12:11 /user/marko/tinkerpop-classic-vertices.ldjson
...
14/06/24 08:16:19 INFO computer.GiraphGraphRunner: GiraphGremlin: TraversalVertexProgram[GiraphGraphStep, VertexStep, VertexStep, ElementValueStep]
14/06/24 08:16:19 INFO mapred.JobClient: Running job: job_201406231706_0015
14/06/24 08:16:20 INFO mapred.JobClient:  map 0% reduce 0%
14/06/24 08:16:41 INFO mapred.JobClient:  map 33% reduce 0%
14/06/24 08:16:42 INFO mapred.JobClient:  map 100% reduce 0%
14/06/24 08:16:43 INFO mapred.JobClient: Job complete: job_201406231706_0015
14/06/24 08:16:43 INFO mapred.JobClient: Counters: 32
14/06/24 08:16:43 INFO mapred.JobClient:   Map-Reduce Framework
14/06/24 08:16:43 INFO mapred.JobClient:     Spilled Records=0
14/06/24 08:16:43 INFO mapred.JobClient:     Map input records=3
14/06/24 08:16:43 INFO mapred.JobClient:     SPLIT_RAW_BYTES=132
14/06/24 08:16:43 INFO mapred.JobClient:     Map output records=0
14/06/24 08:16:43 INFO mapred.JobClient:     Total committed heap usage (bytes)=351272960
14/06/24 08:16:43 INFO mapred.JobClient:   Giraph Timers
14/06/24 08:16:43 INFO mapred.JobClient:     Total (milliseconds)=2897
14/06/24 08:16:43 INFO mapred.JobClient:     Superstep 2 (milliseconds)=67
14/06/24 08:16:43 INFO mapred.JobClient:     Shutdown (milliseconds)=356
14/06/24 08:16:43 INFO mapred.JobClient:     Superstep 1 (milliseconds)=134
14/06/24 08:16:43 INFO mapred.JobClient:     Input superstep (milliseconds)=1014
14/06/24 08:16:43 INFO mapred.JobClient:     Superstep 0 (milliseconds)=298
14/06/24 08:16:43 INFO mapred.JobClient:     Setup (milliseconds)=947
14/06/24 08:16:43 INFO mapred.JobClient:     Superstep 3 (milliseconds)=43
14/06/24 08:16:43 INFO mapred.JobClient:     Superstep 4 (milliseconds)=35
14/06/24 08:16:43 INFO mapred.JobClient:   File Input Format Counters
14/06/24 08:16:43 INFO mapred.JobClient:     Bytes Read=0
14/06/24 08:16:43 INFO mapred.JobClient:   Giraph Stats
14/06/24 08:16:43 INFO mapred.JobClient:     Aggregate finished vertices=0
14/06/24 08:16:43 INFO mapred.JobClient:     Aggregate edges=0
14/06/24 08:16:43 INFO mapred.JobClient:     Sent messages=0
14/06/24 08:16:43 INFO mapred.JobClient:     Current workers=2
14/06/24 08:16:43 INFO mapred.JobClient:     Last checkpointed superstep=0
14/06/24 08:16:43 INFO mapred.JobClient:     Current master task partition=0
14/06/24 08:16:43 INFO mapred.JobClient:     Superstep=5
14/06/24 08:16:43 INFO mapred.JobClient:     Aggregate vertices=6
14/06/24 08:16:43 INFO mapred.JobClient:   FileSystemCounters
14/06/24 08:16:43 INFO mapred.JobClient:     HDFS_BYTES_READ=2488
14/06/24 08:16:43 INFO mapred.JobClient:     FILE_BYTES_WRITTEN=235290
14/06/24 08:16:43 INFO mapred.JobClient:     HDFS_BYTES_WRITTEN=3318
14/06/24 08:16:43 INFO mapred.JobClient:   Job Counters
14/06/24 08:16:43 INFO mapred.JobClient:     Launched map tasks=3
14/06/24 08:16:43 INFO mapred.JobClient:     SLOTS_MILLIS_REDUCES=0
14/06/24 08:16:43 INFO mapred.JobClient:     SLOTS_MILLIS_MAPS=29890
14/06/24 08:16:43 INFO mapred.JobClient:   File Output Format Counters
14/06/24 08:16:43 INFO mapred.JobClient:     Bytes Written=0

Understanding the Output
~~~~~~~~~~~~~~~~~~~~~~~~

When job completes, the output is written to `gremlin.outputLocation` which was defined in the properties file as `output`. Realize that Hadoop will write the output according to the number of workers. Given that there were 2 workers, there are two files in `output`.

[source,bash]
$ hadoop fs -ls /user/marko/output/
Found 1 items
drwxr-xr-x   - marko supergroup          0 2014-06-24 09:03 /user/marko/output/~g
$ hadoop fs -ls /user/marko/output/~g
Found 4 items
-rw-r--r--   1 marko supergroup          0 2014-06-24 09:03 /user/marko/output/~g/_SUCCESS
drwxr-xr-x   - marko supergroup          0 2014-06-24 09:03 /user/marko/output/~g/_logs
-rw-r--r--   1 marko supergroup       1820 2014-06-24 09:03 /user/marko/output/~g/part-m-00001
-rw-r--r--   1 marko supergroup       1498 2014-06-24 09:03 /user/marko/output/~g/part-m-00002


[source,bash]
$ hadoop fs -cat output/~g/*
{"inE":[],"outE":[{"inV":3,"inVLabel":"vertex","outVLabel":"vertex","id":0,"label":"created","type":"edge","outV":1,"hiddens":{},"properties":{"weight":0.4}},{"inV":2,"inVLabel":"vertex","outVLabel":"vertex","id":2,"label":"knows","type":"edge","outV":1,"hiddens":{},"properties":{"weight":0.5}},{"inV":4,"inVLabel":"vertex","outVLabel":"vertex","id":3,"label":"knows","type":"edge","outV":1,"hiddens":{},"properties":{"weight":1.0}}],"id":1,"label":"vertex","type":"vertex","hiddens":{"gremlin.traversalVertexProgram.traverserTracker":{"previousObjectTracks":{},"graphTracks":{},"objectTracks":{},"doneGraphTracks":{},"doneObjectTracks":{}}},"properties":{"name":"marko","age":29}}
{"inE":[{"inV":5,"inVLabel":"vertex","outVLabel":"vertex","id":0,"label":"created","type":"edge","outV":4,"hiddens":{},"properties":{"weight":1.0}}],"outE":[],"id":5,"label":"vertex","type":"vertex","hiddens":{"gremlin.traversalVertexProgram.traverserTracker":{"previousObjectTracks":{"ripple":1},"graphTracks":{},"objectTracks":{},"doneGraphTracks":{},"doneObjectTracks":{"ripple":1}}},"properties":{"name":"ripple","lang":"java"}}
{"inE":[{"inV":3,"inVLabel":"vertex","outVLabel":"vertex","id":0,"label":"created","type":"edge","outV":1,"hiddens":{},"properties":{"weight":0.4}},{"inV":3,"inVLabel":"vertex","outVLabel":"vertex","id":1,"label":"created","type":"edge","outV":4,"hiddens":{},"properties":{"weight":0.4}},{"inV":3,"inVLabel":"vertex","outVLabel":"vertex","id":2,"label":"created","type":"edge","outV":6,"hiddens":{},"properties":{"weight":0.2}}],"outE":[],"id":3,"label":"vertex","type":"vertex","hiddens":{"gremlin.traversalVertexProgram.traverserTracker":{"previousObjectTracks":{"lop":1},"graphTracks":{},"objectTracks":{},"doneGraphTracks":{},"doneObjectTracks":{"lop":1}}},"properties":{"name":"lop","lang":"java"}}
{"inE":[{"inV":4,"inVLabel":"vertex","outVLabel":"vertex","id":8,"label":"knows","type":"edge","outV":1,"hiddens":{},"properties":{"weight":1.0}}],"outE":[{"inV":5,"inVLabel":"vertex","outVLabel":"vertex","id":10,"label":"created","type":"edge","outV":4,"hiddens":{},"properties":{"weight":1.0}},{"inV":3,"inVLabel":"vertex","outVLabel":"vertex","id":11,"label":"created","type":"edge","outV":4,"hiddens":{},"properties":{"weight":0.4}}],"id":4,"label":"vertex","type":"vertex","hiddens":{"gremlin.traversalVertexProgram.traverserTracker":{"previousObjectTracks":{},"graphTracks":{},"objectTracks":{},"doneGraphTracks":{},"doneObjectTracks":{}}},"properties":{"name":"josh","age":32}}
{"inE":[{"inV":2,"inVLabel":"vertex","outVLabel":"vertex","id":7,"label":"knows","type":"edge","outV":1,"hiddens":{},"properties":{"weight":0.5}}],"outE":[],"id":2,"label":"vertex","type":"vertex","hiddens":{"gremlin.traversalVertexProgram.traverserTracker":{"previousObjectTracks":{},"graphTracks":{},"objectTracks":{},"doneGraphTracks":{},"doneObjectTracks":{}}},"properties":{"name":"vadas","age":27}}
{"inE":[],"outE":[{"inV":3,"inVLabel":"vertex","outVLabel":"vertex","id":12,"label":"created","type":"edge","outV":6,"hiddens":{},"properties":{"weight":0.2}}],"id":6,"label":"vertex","type":"vertex","hiddens":{"gremlin.traversalVertexProgram.traverserTracker":{"previousObjectTracks":{},"graphTracks":{},"objectTracks":{},"doneGraphTracks":{},"doneObjectTracks":{}}},"properties":{"name":"peter","age":35}}
...
In the hidden properties of the vertex is a property called `gremlin.traverserTracker`. The `doneObjectTracks`-key states that there is a single counter on the `String` "lop". An analogous result is found on "ripple." This is the correct result of the computation as:
...
The example in the previous section was using a static Java `SSupplier<Traversal>`. This is the method to use when programmatically interacting with Giraph-Gremlin. However, it is also possible to use Giraph-Gremlin via the Gremlin Console.

IMPORTANT: The directory references below will be different depending on the location of `gremlin-console`. Be sure to adjust paths accordingly.
...
$ bin/gremlin.sh
         ...
         (o o)
─────oOOo-(3)-oOOo─────
gremlin> :use install com.tinkerpop giraph-gremlin 3.x.y
==>loaded: [com.tinkerpop, giraph-gremlin, 3.x.y] - restart the console to use [giraph]
gremlin> :q
$ bin/gremlin.sh
...
INFO  com.tinkerpop.gremlin.giraph.process.computer.GiraphGraphRunner  - GiraphGremlin: TraversalVertexProgram[GiraphGraphStep, GroupByStep]
WARN  org.apache.hadoop.mapred.JobClient  - Use GenericOptionsParser for parsing the arguments. Applications should implement Tool for the same.
INFO  org.apache.hadoop.mapred.JobClient  - Running job: job_201406231706_0017
...
INFO  org.apache.hadoop.mapred.JobClient  - Job complete: job_201406231706_0017
INFO  org.apache.hadoop.mapred.JobClient  - Counters: 31
...
INFO  org.apache.hadoop.mapred.JobClient  -     Total committed heap usage (bytes)=243793920
INFO  org.apache.hadoop.mapred.JobClient  -   Giraph Timers
INFO  org.apache.hadoop.mapred.JobClient  -     Total (milliseconds)=7131
INFO  org.apache.hadoop.mapred.JobClient  -     Superstep 2 (milliseconds)=56
INFO  org.apache.hadoop.mapred.JobClient  -     Shutdown (milliseconds)=315
INFO  org.apache.hadoop.mapred.JobClient  -     Superstep 1 (milliseconds)=115
INFO  org.apache.hadoop.mapred.JobClient  -     Input superstep (milliseconds)=5404
INFO  org.apache.hadoop.mapred.JobClient  -     Superstep 0 (milliseconds)=262
INFO  org.apache.hadoop.mapred.JobClient  -     Setup (milliseconds)=923
INFO  org.apache.hadoop.mapred.JobClient  -     Superstep 3 (milliseconds)=54
...
INFO  org.apache.hadoop.mapred.JobClient  -     Superstep=4
INFO  org.apache.hadoop.mapred.JobClient  -     Aggregate vertices=6
INFO  org.apache.hadoop.mapred.JobClient  -   FileSystemCounters
INFO  org.apache.hadoop.mapred.JobClient  -     HDFS_BYTES_READ=2488
INFO  org.apache.hadoop.mapred.JobClient  -     FILE_BYTES_WRITTEN=239055
INFO  org.apache.hadoop.mapred.JobClient  -     HDFS_BYTES_WRITTEN=3504
...
INFO  org.apache.hadoop.mapred.JobClient  -     SLOTS_MILLIS_MAPS=41383
...
INFO  com.tinkerpop.gremlin.giraph.process.computer.GiraphGraphRunner  - GiraphGremlin: GroupByMapReduce
INFO  org.apache.hadoop.mapred.JobClient  - Running job: job_201406231706_0018
INFO  org.apache.hadoop.mapred.JobClient  -  map 0% reduce 0%
INFO  org.apache.hadoop.mapred.JobClient  -  map 100% reduce 0%
INFO  org.apache.hadoop.mapred.JobClient  -  map 100% reduce 33%
INFO  org.apache.hadoop.mapred.JobClient  -  map 100% reduce 100%
INFO  org.apache.hadoop.mapred.JobClient  - Job complete: job_201407241031_0018
INFO  org.apache.hadoop.mapred.JobClient  - Counters: 26
INFO  org.apache.hadoop.mapred.JobClient  -   Map-Reduce Framework
INFO  org.apache.hadoop.mapred.JobClient  -     Spilled Records=12
INFO  org.apache.hadoop.mapred.JobClient  -     Map output materialized bytes=274
...
INFO  org.apache.hadoop.mapred.JobClient  -     Map output bytes=250
INFO  org.apache.hadoop.mapred.JobClient  -     Reduce shuffle bytes=274
...
INFO  org.apache.hadoop.mapred.JobClient  -     Total committed heap usage (bytes)=573571072
INFO  org.apache.hadoop.mapred.JobClient  -   File Input Format Counters
INFO  org.apache.hadoop.mapred.JobClient  -     Bytes Read=6
INFO  org.apache.hadoop.mapred.JobClient  -   FileSystemCounters
INFO  org.apache.hadoop.mapred.JobClient  -     HDFS_BYTES_READ=1513
INFO  org.apache.hadoop.mapred.JobClient  -     FILE_BYTES_WRITTEN=248960
INFO  org.apache.hadoop.mapred.JobClient  -     FILE_BYTES_READ=268
INFO  org.apache.hadoop.mapred.JobClient  -     HDFS_BYTES_WRITTEN=356
INFO  org.apache.hadoop.mapred.JobClient  -   Job Counters
INFO  org.apache.hadoop.mapred.JobClient  -     Launched map tasks=2
INFO  org.apache.hadoop.mapred.JobClient  -     Launched reduce tasks=1
INFO  org.apache.hadoop.mapred.JobClient  -     SLOTS_MILLIS_REDUCES=8593
INFO  org.apache.hadoop.mapred.JobClient  -     Total time spent by all reduces waiting after reserving slots (ms)=0
INFO  org.apache.hadoop.mapred.JobClient  -     SLOTS_MILLIS_MAPS=7088
...
INFO  org.apache.hadoop.mapred.JobClient  -   File Output Format Counters
INFO  org.apache.hadoop.mapred.JobClient  -     Bytes Written=356
...
gremlin> hdfs.head('output/~cap')
==>SEQ:com.tinkerpop.gremlin.giraph.process.computer.KryoWritable:com.tinkerpop.gremlin.giraph.process.computer.KryoWritable�h�p��
�>�]�3�+�a%java.util.ArrayLis�mark�vada�$�ejava.util.ArrayLis�pete�%�ijava.util.ArrayLis�rippl�(�o"java.util.ArrayLis�lo�jos�
...
IMPORTANT: When using Giraph-Gremlin from the Gremlin console in OLAP mode, the only Gremlin language subset supported is Gremlin-Groovy.

A list of the HDFS methods available are itemized below:
 ...
 * `hdfs.exists(String path)`: whether the specification path exists
 * `hdfs.rm(String path)`: remove the specified path
 * `hdfs.rmr(String path)`: remove the specified path with recursion
 * `hdfs.copyToLocal(String from, String to)`: copy the specified path to the specified local path
 * `hdfs.copyFromLocat(String from, String to)`: copy the specified local path to the specified path
 * `hdfs.mergeToLocal(String from, String to)`: merge the files at path to the specified local path
 * `hdfs.head(String path)`: display the data in the path
 * `hdfs.head(String path, long lineCount)`: display only the first `totalKeyValues` in the path
 * `hdfs.head(String path, long totalKeyValues, Class<Writable> writableClass)`: head the path interpreting the key values as respective writable
...
gremlin.pageRankVertexProgram.doResultMapReduce=true
giraph.minWorkers=2
giraph.maxWorkers=2
...
Note that with `gremlin.pageRankVertexProgram.doResultMapReduce=true` a MapReduce job is executed that generates a tab-deliminated file in HDFS where the keys are the ids of the vertices and the values are the double PageRank values.

[source,text]
$ hadoop fs -ls output
Found 2 items
drwxr-xr-x   - marko supergroup          0 2014-07-28 16:27 /user/marko/output/pageRank
drwxr-xr-x   - marko supergroup          0 2014-07-28 16:27 /user/marko/output/~g
~/software/tinkerpop/tinkerpop3/giraph-gremlin$ hadoop fs -cat output/pageRank/*
