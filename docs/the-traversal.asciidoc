...
It is possible to filter vertices, edges, and vertex properties based on their properties.
...
When the objects of the traversal stream need to be sorted, `order()`-step can be leveraged.
...
One of the most traversed objects in a traversal is an `Element`. An element can have properties associated with it (i.e. key/value pairs). In many situations, it is desirable to sort an element traversal stream according to a comparison of their properties. The `orderBy()`-step exists to support this. (*map*)
...
As traversers propagate through the `Traversal`, it is possible to only allow a certain number of them to pass through with `range()`-step (*filter*).
...
==>v[3]
==>v[4]
gremlin> g.V().as('a').both().jump('a',10000).range(6,10)
==>v[1]
==>v[1]
==>v[1]
==>v[1]
