...
[source,groovy]
gremlin> g.V().value('lang')
==>java
==>java
gremlin> g.V().value('lang').dedup()
==>java
...
[source,groovy]
gremlin> g.V().value('name').map{[it.get(), it.get().length()]}
==>[marko, 5]
==>[vadas, 5]
==>[lop, 3]
==>[josh, 4]
==>[ripple, 6]
==>[peter, 5]
gremlin> g.V().value('name').dedup{it.get().length()}
==>marko
==>lop
==>josh
==>ripple
...
==>v[4]
==>v[6]
gremlin> g.v(1).as('x').out('created').in('created').value('name')
==>marko
==>josh
==>peter
gremlin> g.v(1).as('x').out('created').in('created').value('name').except(['marko','stephen'])
==>josh
==>peter
...
[source,groovy]
gremlin> g.v(1).out('knows').value('name')
==>vadas
==>josh
gremlin> g.v(1).out('knows').value('name').fold() <1>
==>[vadas, josh]
gremlin> g.v(1).out('knows').value('name').fold(0){a,b -> a + b.get().length()} <2>
==>9
...
==>v[1]
==>v[2]
gremlin> g.V().interval('age',20,30).value('age')
==>29
==>27
gremlin> g.V().has('age',gt,20).has('age',lte,30).value('age') // the equivalent expression using only has()
==>29
==>27
...
==>ripple
==>vadas
gremlin> g.V().value('name').order(decr)
==>vadas
==>ripple
==>peter
==>marko
==>lop
==>josh
gremlin> g.V().value('name').order{a,b -> a.get()[1] <=> b.get[1]} // order by the second letter
==>marko
==>vadas
==>peter
==>ripple
...
Note that `order()` takes a `Comparator` and thus, its possible to use comparator chaining to do multi-level comparisons.

[source,groovy]
gremlin> import static java.util.Comparator.*
...
gremlin> import java.util.function.*
...
gremlin> g.V().value('name').order(comparing{it.get()[1]}) // order by the second letter
==>marko
==>vadas
==>peter
==>ripple
...
gremlin> g.V().value('name').
           order(comparing{it.get()[1]}.thenComparing((Function){it.get()[2]})) // order by second letter and if a tie, the third letter
==>vadas
==>marko
==>peter
==>ripple
...
==>josh
==>peter
gremlin> g.v(1).as('x').out('created').in('created').value('name').retain(['marko','stephen'])
==>marko
...
==>strategies[TraverserSourceStrategy, LabeledEndStepStrategy, UntilStrategy, DedupOptimizerStrategy, IdentityReductionStrategy, SideEffectCapStrategy, MatchWhereStrategy, ChooseLinearStrategy, UnionLinearStrategy, ComparingReductionStrategy, EngineDependentStrategy]
...
gremlin> GraphTraversalStrategyRegistry.instance().unregister(IdentityReductionStrategy)
==>null
gremlin> t = g.V().identity().identity().identity().range(0,1)
==>v[1]
==>v[2]
