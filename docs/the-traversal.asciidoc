...
gremlin> g.V().as('person').
           properties('location').orderBy('startTime').limit(2).
           value().as('location').select{it.value('name')}{it} <1>
==>[person:daniel, location:spremberg]
==>[person:stephen, location:centreville]
gremlin> g.V().as('person').
           local(g.of().properties('location').orderBy('startTime').limit(2)).
           value().as('location').select{it.value('name')}{it} <2>
...
==>lop
==>josh
gremlin> g.V().values('name').order(comparing{it.get()[1]},comparing{it.get()[2]}) // order by second letter and if a tie, the third letter
==>vadas
==>marko
==>peter
==>ripple
...
==>ripple
==>peter
gremlin> g.V().orderBy('name').values('name')
==>josh
==>lop
==>marko
==>peter
==>ripple
==>vadas
gremlin> g.V().orderBy('name',decr).values('name')
==>vadas
==>ripple
==>peter
==>marko
