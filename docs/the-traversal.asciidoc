...
A traverser can contain a traverser-local sideEffect data structure called a "sack". The `sack()`-step is used to manipulate traverser sacks (*sideEffect* or *map*). Each sack of each traverser is created when the traverser is generated using `withSack(initialValueSupplier,splitOperator?,mergerOperator?)`.

* *Initial value supplier*: A `Supplier` providing the initial value of each traverser's sack.
* *Split operator*: a `UnaryOperator` that clones the traversers sack when the traverser splits. If no split operator is provided, then it assumes `UnaryOperator.identity()`.
* *Merge operator*: a `BinaryOperator` that merges two traverser sacks when the traversers merge. The merge operator should be link:http://en.wikipedia.org/wiki/Associative_property[associative], link:http://en.wikipedia.org/wiki/Commutative_property[commutative], and link:http://en.wikipedia.org/wiki/Distributive_property[distributive]. If no merge operator is provided, traversers can not be merged.

In the first example below, for each traverser created at each vertex in the graph (`g.V()`), a 1.0 sack is supplier (`withSack({1.0f})`). and then accessed (`sack()`). In the second example, a random float supplier is used to generate sack values.
...
When complex objects are used (i.e. non-primitives), then a split operator should be defined to ensure that each traverser gets a clone of his parent's sack. The first example does not use a split operator and as such, the same map is propagated to all traversers (a global data structure). For such global data structures, `withSideEffect()` should be used. The second example, demonstrates how `Map.clone()` ensures that each traverser's sack contains a unique, local sack.
...
Finally, the merge operator is important in situations when two traversers are merged. When merging, their bulk count is summed and their sacks are merged. Assuming an associative, distributive, and commutative merge operator, the semantics of the traversal are respected.

[source,groovy]
gremlin> g.V().withSack({1.0f},sum).as('x').outE().sack(mult,'weight').inV().jump('x',2)
==>v[5]
==>v[3]
gremlin> g.V().withSack({1.0f},sum).as('x').outE().sack(mult,'weight').inV().jump('x',2).sack()
==>1.0
==>0.4
gremlin> g.V().withSack({1.0f},sum).as('x').outE().sack(mult,'weight').inV().jump('x',2).path{it}{it.value('weight')}
==>[v[1], 1.0, v[4], 1.0, v[5]]   // 1.0 * 1.0 * 1.0 = 1.0
==>[v[1], 1.0, v[4], 0.4, v[3]]   // 1.0 * 1.0 * 0.4 = 0.4

CAUTION: If no merge operator is provided, then traversers with sacks will not be able to be merged. The drawback of this is readily seen in OLAP computations where traversers must be enumerated instead of counted (see <<distributed-gremlin-gotchas,Distributed Gremlin Gotchas>>). Note that various OLTP situations make use of the traverser merge optimization.
