...
| `branch().by(Function<Traverser<S>,Collection<String>>)` | split the traverser to all steps labeled by returned string collection.
|=========================================================
...
[source,groovy]
gremlin> g.V().branch().by{it.get().value('name') == 'marko' ? ['a'] : ['b'] }.as('a').
