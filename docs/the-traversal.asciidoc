...
MatchStep brings functionality similar to link:http://en.wikipedia.org/wiki/SPARQL[SPARQL] to Gremlin. Like SPARQL, MatchStep conjoins a set of patterns applied to a graph.  For example, the following traversal finds exactly those songs which Jerry Garcia has both sung and written:  

[source,groovy]
gremlin> g.V().match('a',
gremlin>     g.of().as('a').has('name', 'Garcia'),
gremlin>     g.of().as('a').in('writtenBy').as('b'),
gremlin>     g.of().as('a').in('sungBy').as('b')).select('b').name
==>CREAM PUFF WAR
==>CRYPTICAL ENVELOPMENT

Among the features which differentiate MatchStep from SPARQL are:
