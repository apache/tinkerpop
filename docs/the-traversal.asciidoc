...
gremlin> g.V().group().by{it.value('name')[1]}.by('name')   // for each vertex in the group, get their name (now the name[1] is apparent)
==>[a:[marko, vadas], e:[peter], i:[ripple], o:[lop, josh]]
gremlin> g.V().group().by{it.value('name')[1]}.by('name').by{it.size()}   // for each grouping, what is it's size?
==>[a:2, e:1, i:1, o:2]
...
==>1.0
==>0.4
gremlin> g.V().withSack{1.0f}.repeat(__.outE().sack(mult,'weight').inV()).times(2).path().by().by('weight')
