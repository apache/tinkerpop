...
<2> The `select{it.value('name')}` projects each binding vertex to their name property and `where()` operates to ensure `a` and `b` strings are not the same.
<3> The first `select()` projects a vertex binding set. A binding is filtered if 'a' vertex equals 'b' vertex. A binding is filtered if 'a' doesn't know 'b'. The final `select()` projects to the name of the vertices.
