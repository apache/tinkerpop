...
[width="100%",cols="12,10",options="header"]
|=========================================================
| Step| Description
| `map(Function<Traverser<S>,E>)` | map the traverser to some object of type `E` for the next step to process.
| `map(BiFunction<Traverser<S>, Traversal.SideEffects,E>)` | like previous, but access to traversal side effects is allowed.
| `flatMap(Function<Traverser<S>, Iterator<E>>)` | map the traverser to an iterator of `E` objects that are streamed to the next step.
| `flatMap(BiFunction<Traverser<S>, Traversal.SideEffects, Iterator<E>>)` | like previous, but access to traversal side effects is allowed.
...
| `filter(BiPredicate<Traverser<S>, Traversal.SideEffects>)` | like previous, but access to traversal side effects is allowed.
| `sideEffect(Consumer<Traverser<S>>)` | perform some operation on the traverser and pass it to the next step.
| `sideEffect(BiConsumer<Traverser<S>, Traversal.SideEffects>)` | like previous, but access to traversal side effects is allowed.
|=========================================================

If a `BiXXX`-lambda is provided, then the user has access to `Traversal.SideEffects` which contains side effect data structures as well as objects at particular labeled steps. A series of examples are provided below.
