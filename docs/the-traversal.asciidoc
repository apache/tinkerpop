...
[source,groovy]
gremlin> TraversalMetrics.isEnabled()
==>false
gremlin> TraversalMetrics.enable()
==>null
gremlin> g.V().out().out().profile()
==>
...
gremlin> TraversalMetrics.disable()
==>null
gremlin> g.V().out().out().profile()
...
There are various provided strategies that are required of `GraphTraversal` in order for it to function properly. However, not all such strategies are required and in fact, depending on _a priori_ knowledge of the statistics of the graph being traversed, turning off default strategies and/or activating custom strategies may prove fruitful. `GraphTraversalStrategyRegistry` is a global static class that supports the registering (and un-registering) traversal strategies that will be applied every time a `GraphTraversal` is constructed (e.g. `DefaultGraphTraversal`).
