...
[source,groovy]
gremlin> g.V(1).repeat(g.of().out()).until{it.loops()>=2}.values('name') <1>
==>ripple
==>lop
gremlin> g.V().until{it.get().value('name').equals('ripple')}.repeat(g.of().out()).path().by('name') <2>
==>[marko, josh, ripple]
==>[josh, ripple]
==>[ripple]
...
If a predetermined number of loops is known, it is possible to simply provide the `until()` an integer (e.g. repeat twice).

[source,groovy]
gremlin> g.V(1).repeat(g.of().out()).until(2).values('name')
==>ripple
==>lop
...
The `repeat()`-step also supports an "emit predicate", where the predicate for an empty argument `emit()` is `true`. Whereby the traverser is split in two -- the traverser exits the code block as well as continues back within the code block.
