...
==>v[4]
==>v[6]
gremlin> g.v(1).out('co-developer').name
==>josh
==>peter
...
==>v[3]
==>v[3]
gremlin> g.v(1).out('created').aggregate('x').in('created').out('created').except('x').name // what have marko's collaborators created that he hasn't created?
==>ripple
...
[source,groovy]
gremlin> g.v(4).out.name.inject('daniel')
==>ripple
==>lop
==>daniel
gremlin> g.v(4).out.name.inject('daniel').map{it.get().length()}
==>6
==>3
==>6
gremlin> g.v(4).out.name.inject('daniel').map{it.get().length()}.path
==>[v[4], v[5], ripple, 6]
==>[v[4], v[3], lop, 3]
==>[daniel, 6]
...
==>2
==>3
gremlin> g.of().inject(1,2).map{it.get() + 1}.map{g.v(it.get())}.name
==>vadas
==>lop
...
[source,groovy]
gremlin> path = g.v(1).as('a').has('name').as('b').out('knows').out('created').as('c').has('name','ripple').name.as('d').identity.as('e').path.next()
==>[v[1], v[4], v[5], ripple]
gremlin> path.size()
==>4
...
[source,groovy]
gremlin> g.V.aggregate('x')[0..1].cap('x')
==>[v[1], v[2], v[3], v[4], v[5], v[6]]
gremlin> g.V.store('x')[0..1].cap('x')
==>[v[1], v[2], v[3]]
