...
[source,groovy]
gremlin> g.V(1).repeat(__.out()).until(__.has('lang')).path().by('name') <1>
==>[marko, lop]
==>[marko, josh, ripple]
==>[marko, josh, lop]
gremlin> g.V(1).emit(__.has('age')).repeat(__.out()).path().by('name') <2>
==>[marko]
==>[marko, vadas]
==>[marko, josh]
...
<1> Starting from vertex 1, keep taking outgoing edges until a software vertex is reached (i.e. a vertex with a `lang` property).
<2> Starting from vertex 1, and in an infinite loop, emit the vertex if it is a person (i.e. has an `age` property) and then traverser the outgoing edges.
