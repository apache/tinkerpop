...
g.V().out('knows').map{it.value('name') + ' is the friend name'} <2>
g.V().out('knows').sideEffect{println it} <3>
g.V().as('person').out('knows').as('friend').select{it.value('name')} <4>

[source,java]
g.V().out("knows").values("name") <1>
g.V().out("knows").map(v -> v.value("name") + " is the friend name") <2>
g.V().out("knows").sideEffect(System.out::println) <3>
g.V().as("person").out("knows").as("friend").select(v -> ((Vertex)v).value("name")) <4>
...
<3> Java supports link:https://docs.oracle.com/javase/tutorial/java/javaOO/methodreferences.html[method references] as lambdas which is not possible in Gremlin-Groovy.
