...
[source,groovy]
gremlin> marko.repeat(g.of().out()).until(2).values('name')
==>ripple
==>lop

IMPORTANT: In TinkerPop2, the `repeat()`-step was called `loop()`. Repeat has been generalized to support both do-while and while-do semantics which is explained in <<repeat-step,Repeat Step>>.
...
g.V().as('person').out('knows').as('friend').select{it.get().value('name')} <4>
...
g.V().out("knows").sideEffect(System.out::println) <3>
g.V().as("person").out("knows").as("friend").select((Function<Vertex, String>) v -> v.value("name")) <4>
