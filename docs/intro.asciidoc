...
g.V().repeat(g.<Vertex>of().out()).until(2) <5>
...
<5> Start-less traversals generated via `Graph.of()` require a start typing in Java, but not in Groovy.
