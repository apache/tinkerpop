...
gremlin> g = TinkerGraph.open()
==>tinkergraph[vertices:0 edges:0]
gremlin> partitionStrategy = new PartitionStrategy("com.example.partition", "A")
==>partitionstrategy
...
gremlin> g = TinkerGraph.open()
==>tinkergraph[vertices:0 edges:0]
gremlin> sg = g.strategy(new ReadOnlyStrategy())
...
[source,groovy]
gremlin> sg = g.strategy(new ReadOnlyStrategy(), new PartitionStrategy("com.example.partition","A"))
