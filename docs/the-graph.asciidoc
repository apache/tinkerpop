...
In TinkerPop3, `MetaProperty<V>` is introduced. All the properties of a `Vertex` are a `MetaProperty`. A `MetaProperty` is like a `Property` in that it has a key/value, but it extends `Property` whereby it can also have a collection of key/value pairs. Moreover, while an `Edge` can only have one property of key "name" (for example), a `Vertex` can have multiple "name" meta-properties. With meta-properties, two features of vertex properties are introduced:

 . Multiple properties: a `Vertex` property key can have multiple values (i.e. a vertex can have multiple "name" properties).
 . Properties on properties: a `Vertex` property can have properties (i.e. a vertex property can have key/value data associated with it).

A running example using meta-properties is provided below to demonstrate and explain the API.
...
==>mp[name->marko]
==>mp[name->marko a. rodriguez]
gremlin> v.properties('name')
==>mp[name->marko]
==>mp[name->marko a. rodriguez]
gremlin> mp = v.properties('name').has(value,'marko').next()
==>mp[name->marko]
gremlin> mp.property('acl','private') <3>
==>p[acl->private]
gremlin> mp = v.properties('name').has(value,'marko a. rodriguez').next()
==>mp[name->marko a. rodriguez]
gremlin> mp.property('acl','public')
==>p[acl->public]
...
==>marko
gremlin> mp = v.properties().next()
==>mp[name->marko]
gremlin> mp.properties()
==>p[acl->private]
gremlin> mp.property('date',2014) <5>
==>p[date->2014]
gremlin> mp.property('creator','stephen')
==>p[creator->stephen]
gremlin> mp.properties()
==>p[date->2014]
==>p[creator->stephen]
...
<2> If a vertex has multiple meta-properties for the same key, then `Vertex.property()` is not allowed.
<3> A vertex meta-property can have standard key/value properties attached to it.
<4> Meta-property removal is identical to property removal.
<5> A vertex meta-property can have any number of key/value properties attached to it.
 ...
 * *Permissions*: Meta-properties can have key/value ACL-type permission information associated with them.
 * *Auditing*: When a vertex meta-property is manipulated, it can have key/value information attached to it saying who the creator, deletor, etc. are.
 * *Provenance*: The "name" of a vertex can be declared by multiple users.

If the concept of meta-properties is difficult to grasp, then it may be best to think of meta-properties in terms of "literal vertices." A vertex can have an edge to a "literal vertex" that has a single value key/value -- e.g. "value=okram." The edge that points to that literal vertex has an edge-label of "name." The properties on the edge represent the literal vertex's properties. The "literal vertex" can not have any other edges to it (only one from the associated vertex).
