...
The above code specifies the creation of a `TinkerGraph` with the use of `ReadOnlyStrategy`.  Note that the `GraphFactory` does not return a `TinkerGraph` instance.  It returns a `StrategyGraph` which is a traverser for the underlying `TinkerGraph` and the `ReadOnlyStrategy`.  Since `StrategyGraph` implements the `Graph` interface, it can be used in the same manner as `TinkerGraph` or any other implementation.
...
gremlin> g = Neo4jGraph.open('/tmp/neo4j')
==>neo4jgraph[EmbeddedGraphDatabase [/tmp/neo4j]]
gremlin> sg = new StrategyGraph(g)
==>identitygraphstrategy[neo4jgraph[EmbeddedGraphDatabase [/tmp/neo4j]]]
gremlin> sg.setGraphStrategy(IdStrategy.build("idKey").create())
==>null
...
gremlin> sg.setGraphStrategy(strategy)
==>null
----
...
StrategyGraph sg = new StrategyGraph(g);
sg.setGraphStrategy(strategy);
