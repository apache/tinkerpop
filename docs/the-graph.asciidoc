...
g.io().writeGraphML("tinkerpop-classic.xml");

final Graph newGraph = TinkerGraph.open();
newGraph.io().readGraphML("tinkerpop-classic.xml")'
----
...
----
final Graph g = TinkerFactory.createClassic();
g.io().writeGraphSON("tinkerpop-classic.json");

final Graph newGraph = TinkerGraph.open();
newGraph.io().readGraphSON("tinkerpop-classic.json")'
----
...
try (final OutputStream os = new FileOutputStream("tinkerpop-classic.json")) {
    g.io().graphSONWriter().normalize(true).create().writeGraph(os);
}
...
==>java.io.FileOutputStream@43034809
gremlin> g.io().graphSONWriter().embedTypes(true).create().writeVertex(f, g.V(1).next(), BOTH)
==>null
gremlin> f.close()
----
...
----
final Graph g = TinkerFactory.createClassic();
g.io().writeKryo("tinkerpop-classic.gio");

final Graph newGraph = TinkerGraph.open();
newGraph.io().readKryo("tinkerpop-classic.gio")'
----
...
try (final OutputStream os = new FileOutputStream("tinkerpop-classic.gio")) {
    g.io().kryoWriter().normalize(true).create().writeGraph(os);
}
