...
[source,groovy]
g.v(2).in.out('knows')
      .filter{it.get().value('age') > 30}.outE('created')
      .filter{it.get().value('weight') > 0.5}.inV

To visualize it insert the appropriately named `store.as("n")` steps where n is an integer, and the vertices will be highlighted in ascending store step order. 

[source,groovy]
gremlin> g.v(2).in('knows').out('knows').filter{it.get().value('age') > 30}
               .outE('created').filter{it.get().value('weight') > 0.5}.inV.name
==>ripple
gremlin> traversal = g.v(2).store.as('1')
                           .in('knows').store.as('2')
                           .out('knows').filter{it.get().value('age') > 30}.store.as('3')
                           .outE('created').filter{it.get().value('weight') > 0.5}.inV.store.as('4')
==>[v[5]]
