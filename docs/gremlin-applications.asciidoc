...
gremlin> :> g.V().has('name','marko').out('created').value('name')
==>Item{resultItem=lop class=java.lang.String}
...
The above JSON represents the "body" of the request to send to Gremlin Server. When sending this "body" over websockets you can configure your packet frame to use a "text" (1) opcode. If you do so you can just place the body in the packet payload and Gremlin Server will interpret it as being JSON.
If you set the packet frame opcode to "binary" (2) Then you will also have to add a "header" to your "body".  The "header" basically consists of a "mime type" so that Gremlin Server knows how to deserialize the `RequestMessage`. So, the actual byte array sent to Gremlin Server would be formatted as follows:
...
'It is worth noting that Gremlin Server will only accept masked packets.'
...
|type |Helps identify the "type" of value in the `result` - will be an integer value where "0" signifies an "object", "1" signifies a "collection", and "2" signifies an "empty" result.
|result |The return value as classified by the `resultType`.
|========================================================= 

Gremlin Server will send a single response when the `resultType` is of type object (`resultType=0`) or empty (`resultType=2`). In this case the `ResponseMessage` returned to the client would look something like this:
...
Gremlin Server is capable of streaming results when the `resultType` is of type collection (`resultType=1`). When Gremlin Server is streaming results, additional responses will arrive over the WebSocket until the iteration of the result on the server is complete.  Each successful incremental message will have a `ResultCode` of 200. The response stream is always marked as "complete" by a terminating message.  The "terminator" will look like this:
...
opProcessors and their arguments
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Bellow is a non-exhaustive list of available operation and args for embedded opProcessors.

*args common to all opProcessors*

[width="100%",cols="2,2,9",options="header"]
|=========================================================
|Key |Type |Description
|batchSize |Int |When the result is an iterator this defines the number of iterations each result packet should contain. 
|=========================================================
...
[width="100%",cols="3,a",options="header"]
|=========================================================
|Key |Description
|processor |''
...
|=========================================================
|Key |Type |Description
|gremlin |String | *Required* The gremlin script that you wish to evaluate against gremlin server
|bindings |Collection |A key value collection of bindings
|language |String |The flavor used. ex: `gremlin-groovy`
|=========================================================
...
|Key |Type |Description
|gremlin |String | *Required* The gremlin script that you wish to evaluate against gremlin server
|session |String | *Required* The session id for the current session. (the session will be created if it doesn't exist)
|bindings |Collection |A key value collection of bindings
|language |String |The flavor used. ex: `gremlin-groovy`
|=========================================================
...
'`import` operation arguments'
[width="100%",cols="2,2,9",options="header"]
|=========================================================
|Key |Type |Description
|imports |NA | *Required* NA
|=========================================================
...
|=========================================================
|Key |Type |Description
|infoType |NA | *Required* NA
|=========================================================

'`use` operation arguments'
[width="100%",cols="2,2,9",options="header"]
|=========================================================
|Key |Type |Description
|coordinates |NA | *Required* NA
|=========================================================
...
[source,groovy]
g.v(2).in().out('knows').
      has('age',T.gt,30).outE('created').
      has('weight',T.gt,0.5f).inV()
...
[source,groovy]
gremlin> g.v(2).in('knows').out('knows').has('age',T.gt,30).
               outE('created').has('weight',T.gt,0.5f).inV().value('name')
==>ripple
gremlin> traversal = g.v(2).store('1').
                           in('knows').store('2').
                           out('knows').has('age',T.gt,30).store('3').
                           outE('created').has('weight',T.gt,0.5f).inV().store('4')
==>[v[5]]
...
. Missing traversal field: if a traversal field is called and that field doesn't exist, then it is assumed either a step() or defaults to `value('step')`.
