...
[source,groovy]
----
gremlin> :remote connect server conf/remote.yaml
...
----
gremlin> :> g.V().value('name')
==>Result{resultItem=marko class=java.lang.String}
==>Result{resultItem=vadas class=java.lang.String}
==>Result{resultItem=lop class=java.lang.String}
==>Result{resultItem=josh class=java.lang.String}
==>Result{resultItem=ripple class=java.lang.String}
==>Result{resultItem=peter class=java.lang.String}
gremlin> :> g.V().has('name','marko').out('created').value('name')
==>Result{resultItem=lop class=java.lang.String}
gremlin> :> g.E().label().groupCount()
==>Result{resultItem={created=4, knows=2} class=java.lang.String}
----
...
[source,groovy]
----
gremlin> :remote connect server conf/remote-objects.yaml <1>
==>connected - localhost/127.0.0.1:8182
...
==>Result{resultItem={created=4, knows=2} class=java.util.HashMap}
...
<4> The last result of a remote script is always stored in the reserved variable `_l`, which allows access to the `Result` and by virtue of that, the `Map` itself.
...
gremlin> g.v(2).in('knows').out('knows').has('age',gt,30).
               outE('created').has('weight',gt,0.5f).inV().value('name')
==>ripple
