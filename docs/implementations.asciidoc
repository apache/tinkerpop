...
image:hadoop-logo-notext.png[width=100,float=left] link:http://hadoop.apache.org/[Hadoop] is a distributed computing framework that can be used to process files so large they are represented across a distributed file system known as HDFS. When those files represent TinkerPop3 graphs, then Hadoop-Gremlin can be used to process such graphs -- both in OLTP and OLAP form.
