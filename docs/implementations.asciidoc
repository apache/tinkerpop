...
|gremlin.deriveMemory |Whether or not `Memory` is yielded (requires an extra MapReduce job if true).
...
TIP: image:gremlin-sugar.png[width=50,float=left] For those wanting to use the <<sugar-plugin,SugarPlugin>> with their submitted traversal, simply `:remote config useSugar true` as well as be `:plugin use sugar` at the start of the Gremlin Console session if it is not already loaded.
