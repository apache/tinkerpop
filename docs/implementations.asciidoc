...
Below demonstrates the runtime benefits of indices and demonstrates how if there is no defined schema index (only vertex labels), a linear scan of vertex-label partition is still faster than linear scan of all vertices.
