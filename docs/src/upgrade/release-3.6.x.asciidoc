...
==== GraphBinary set as the default serialization format for gremlin-python and Gremlin.NET

Python and .NET have support for GraphBinary at least since 3.5.0 but kept GraphSON 3 by default. It now seems safe 
to make GraphBinary the default in 3.6.x.

To continue using the GraphSon, explicitly specify the serializer.
