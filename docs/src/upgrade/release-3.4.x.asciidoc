...
fails to start if all graphs fails. This can be usefull for instance on a kubernetes cluster
to ensure that pop wil be restarted as it cannot handle properly requests. 
DefaultGraphManager is no more final and thus can be extended.

link:https://issues.apache.org/jira/browse/TINKERPOP-2436[TINKERPOP-2436]
