...
remaining in the `Buffer` presented to the deserializer, is rejected with an `IOException` rather than driving a large
allocation. A message too short to contain a declared prefix, and a negative `BulkSet` repeat count, are likewise
rejected with an `IOException`. Growable collections and maps are also no longer pre-sized to the full declared count,
so an in-range but large count cannot reserve a large backing array up front. Well-formed messages are unaffected.
