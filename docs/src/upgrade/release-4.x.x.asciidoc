...
The Python driver now streams GraphBinary results directly from the HTTP response body. Results are available to the
caller as they arrive from the server, rather than waiting for the full response to be buffered in memory. This improves
...
- The transport method `read()` has been replaced with `get_stream()`. Custom transport implementations must
  update accordingly.
...
The `AbstractBaseTransport` abstract base class and the `transport_factory` extension point have been removed. Custom
transport implementations are no longer supported. The driver uses `AiohttpHTTPTransport` directly.
