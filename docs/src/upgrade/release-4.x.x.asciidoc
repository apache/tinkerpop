...
JavaScript has a single `Number` type (IEEE 754 double) which loses the distinction between JVM numeric types like
...
On the *deserialization* side, a new `numberMode: 'precise'` connection option wraps incoming numeric values in the
