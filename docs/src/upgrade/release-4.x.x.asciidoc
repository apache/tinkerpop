...
A few Gremlin patterns that worked only because `Tree` was a `Map` (for example `select(keys)`, `count(local)`,
and `unfold()` applied to a `Tree`) no longer compose. Process the `Tree` result client-side after `next()`, or
reshape the upstream traversal.
...
GraphBinary (`0x2b`) wire formats are unchanged, so no serialized data or cross-version compatibility is affected
— only the in-memory Java type changed.
