...
The concrete step must bind each named variable from each result row into the traverser's path history using
`addLabels()`, so that downstream `select()` steps can retrieve them by name. Anonymous pattern elements must
not be exposed in the path. The step must declare `TraverserRequirement.LABELED_PATH` in `getRequirements()`.
