...
As of this release, TinkerPop does not offer a general purpose processing engine for queries that might be given to
`match`. It is up to the underlying graph database to offer support for one. While there is no general purpose
processing engine that can be applied to every graph, TinkerPop does offer limited GQL support with TinkerGraph. It is
therefore possible to try this functionality while graph providers work toward supporting the new `match` step in their
TinkerPop 4.0.0 implementations.
...
See `TinkerGraphMatchStep` and `TinkerGraphDeclarativeMatchStrategy` in `tinkergraph-gremlin` for a complete
reference implementation to study before writing your own.
