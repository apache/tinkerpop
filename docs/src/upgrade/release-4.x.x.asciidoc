...
reshape the upstream traversal. The old `count(local)` (which returned the root-node count) is replaced by
`rootNodes().size()` for the root count, or `nodeCount()` for the total node count.
