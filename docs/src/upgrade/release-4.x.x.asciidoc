...
- The built-in response serializer is `GraphBinarySerializersV4`. The receive path dispatches through the
  configured `response_serializer`, so a custom serializer can be supplied by implementing `version` (the
  Content-Type negotiated in the `Accept` header) and `deserialize_response_stream(stream)` — a generator
  that reads the response body and yields each result item, raising `GremlinServerError` if the response
  ends with a non-success status.
