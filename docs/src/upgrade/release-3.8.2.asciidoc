...
==== Vertex propertyMap() Helper

The Gremlin Language Variants now offer a helper on the `Vertex` type that views a vertex's properties as a map keyed
by property key, complementing the flat list returned by `properties`. `propertyMap()` (`gremlin-javascript`),
`PropertyMap()` (`gremlin-dotnet`), and `property_map()` (`gremlin-python`) each return a map of property key to the
list of that key's `VertexProperty` objects. The default flat-list representation of `properties` is unchanged.
