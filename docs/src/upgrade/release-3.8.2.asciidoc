...
Before 3.8.2, GraphSON 2.0 and 3.0 deserialization loaded a custom `TraversalStrategy` from the application class path
based on its serialized class name. Starting with 3.8.2, GraphSON only deserializes strategies registered with
`TraversalStrategies.GlobalCache`.

Providers that send custom strategies over GraphSON must register every strategy before input is deserialized. Either
