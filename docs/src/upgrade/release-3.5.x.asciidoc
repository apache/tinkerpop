...
* `hasLabel(null)` and `has(T.label,null)` - Formerly, these methods threw general `NullPointerException` failures,
but now filter results. As a `T.label` can't be `null`, it will effectively filter all elements.
