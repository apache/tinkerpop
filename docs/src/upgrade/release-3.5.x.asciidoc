...
Deprecated and removed functionality of the `connectOnStartup` option for Gremlin Javascript to resolve potential `unhandledRejection` and race conditions. Setting `connectOnStartup` to `true` will only trigger a console warning.
