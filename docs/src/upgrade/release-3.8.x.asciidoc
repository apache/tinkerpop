...
The `choose().option(Traversal, v)` is rarely used in comparison to the other overloads with constants, predicates and Pick
tokens. It is also not the recommended approach for performance reasons. In addition, the current implementation leads to
confusion as it only evaluates if the traversal is productive, rather than performing comparison on based on the traversal's
output value. Fixing this behavior would require extensive changes, as such we are removing this functionality from
`choose()` in 3.8.0, with plans to reintroduce a properly implemented version in the next major release.
