...
changes primarily focus on standardizing scoping (local vs global) behavior, simplifying grammar syntax, and enhancing
type safety. You should review the following table that shows the impact level of each change and use that as a guide to
understand how breaking changes may affect your traversals. Also take note of the new steps added to get a sense of how
...
|Breaking |Strategy Change |<<stricter-repeatunrollstrategy,Stricter RepeatUnrollStrategy>>
...
|Breaking |Type Semantics |<<simplified-comparability-semantics,Simplified Comparability Semantics>>
