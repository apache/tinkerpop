...
Windsurf. Through this integration, graph structure can be discovered, Gremlin traversals can be executed, and common
import/export tasks (for example, GraphSON or CSV) can be performed. Basic health checks are included to validate
connectivity.
...
This feature is *experimental* and its interfaces and behavior may change based on feedback in future versions. As a
result, there is no official 3.8.0 release for it, and it is only offered as a 3.8.0-alpha1.
