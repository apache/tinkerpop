...
The semantics of globally-scoped `limit()`, `skip()`, and `range()` steps inside `repeat()` have been modified to ensure 
consistent semantics across repeat iterations.Previously, these steps would track global counters, leading to unexpected 
behavior where subsequent iterations of `limit()` and `range()` would filter all traversers as previous iterations 
reached the global limits. In the case of `skip()`, traversers would be passed to subsequent iterations when they should 
...
This change ensures that globally-scoped `limit()`, `skip()`, and `range()` steps inside `repeat()` now use per-iteration 
counters, making the behavior consistent with manually unrolled traversals. Locally-scoped variants of these steps are 
