...
The semantics of `limit()` and `range()` steps inside `repeat()` has been modified to ensure consistent semantics across 
repeat iterations. Previously, these steps would track global counters, leading to unexpected behavior where subsequent 
iterations would be filtered out entirely as previous iterations reached the global limits. As of 3.8.0 the counters are
tracked per iteration.

[source,groovy]
----
// 3.7.4 - no results due to global limit counter
gremlin> g.withoutStrategies(RepeatUnrollStrategy).V().has('name','JAM').repeat(out('followedBy').limit(2)).times(2).values('name')
gremlin>
gremlin> g.withoutStrategies(RepeatUnrollStrategy).V().has('name','DRUMS').repeat(in('followedBy').range(1,3)).times(2).values('name')
gremlin>

// 3.8.0 - produces results as limit counters tracked per iteration
...
gremlin> g.V().has('name','DRUMS').repeat(in('followedBy').range(1,3)).times(2).values('name')
==>DEAL
==>WOMEN ARE SMARTER
----

This fix ensures that `limit()` and `range()` steps inside `repeat()` now use per-iteration counters, making the 
behavior consistent with manually unrolled traversals. The change affects both global and local variants of these steps 
when used within repeat contexts.

The implementation introduces per-iteration counter tracking in `RangeGlobalStep` that isolates counters based on the 
repeat step context and iteration number. This ensures that each repeat iteration starts with fresh counters, providing 
