...
This inconsistency is now resolved. Both GLVs will deserialize element properties into lists of property objects,
...
----
# 3.7 and before:
v = g.with_("materializeProperties", "tokens").V(1).next() # skip properties with token
# graphson returned v.properties as None, and graphbinary returned v.properties as empty lists
