...
Previously the use of Vertex/ReferenceVertex existed in the grammar, these have now been removed. Going forward, vertex ID should 
...
gremlin> v2 = g.V(2).next();
==>v[2]
gremlin> g.V(v1.id()).outE().where(inV().id().is(v2.id()))
==>e[7][1-knows->2]

AddEdgeStep has been updated to detect an invalid vertex ID. In the case of an invalid id given the following
error will be raised...

"The value given to addE(%s).[to][from]() must resolve to a Vertex or the ID of a Vertex present in the graph.
The provided value does not match any vertices in the graph"
