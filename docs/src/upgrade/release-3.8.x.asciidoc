...
Usage of `dedup()` inside `repeat()` is an example of a traversal that will no longer be unrolled. The following results 
returned from the `grateful-dead` graph demonstrate the change of semantics if the `dedup()` in `repeat()` were to be unrolled:

[source,text]
----
gremlin> g.V(1).repeat(out().dedup()).times(2).order().limit(5)
==>v[1]
==>v[9]
==>v[10]
==>v[11]
==>v[12]
gremlin> g.withoutStrategies(RepeatUnrollStrategy).V(1).out().dedup().out().dedup().order().limit(5)
==>v[1]
==>v[3]
==>v[4]
==>v[5]
