...
*Automatic Pass-through for Unproductive Predicates*

The `choose()` step now automatically passes through traversers when the choice traversal is unproductive. This means
that elements that don't have the properties being evaluated will continue through the traversal unchanged, rather than
being filtered out.
