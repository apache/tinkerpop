...
Any math operations such as Add, Sub, Mul & Div will automatically promote to next number type if an overflow is detected. For integers the promotion sequence is byte->short->integer->long->overflow exception. For floating point the sequence is float->double->infinity.
Previously on overflow values would wrap around their min/max boundaries which is not recommended as would give users unexpected results.
