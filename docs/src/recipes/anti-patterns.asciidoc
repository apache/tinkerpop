...
NOTE: `ByModulatorOptimizationStrategy` is present to automatically translate this anti-pattern to their more performant
 versions for most cases however, it is still best to write Gremlin according to the contents that follow.
...
link:https://tinkerpop.apache.org/docs/x.y.z/recipes/#multi-label-vertices[Multi-Label Vertices] recipe for further
details.
