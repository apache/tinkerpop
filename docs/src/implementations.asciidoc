...
Note that `GiraphGraphComputer` and `SparkGraphComputer` are loaded via their respective plugins. Due to jar conflicts, both Giraph and Spark can not be loaded at the same time.
