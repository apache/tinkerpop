...
####################################
gremlin.hadoop.graphInputRDD=org.apache.tinkerpop.gremlin.spark.process.computer.io.InputRDDFormat
gremlin.hadoop.graphOutputRDD=org.apache.tinkerpop.gremlin.spark.process.computer.io.OutputRDDFormat
...
Spark-Gremlin Provides specific properties in the table below.
[width="100%",cols="2,10",options="header"]
|========================================================
|Property |Description
|gremlin.hadoop.graphInputRDD |A class for creating RDD's from underlying graph data, defaults to Hadoop `InputFormat`
|gremlin.hadoop.graphOutputRDD |A class for output RDD's, defaults to Hadoop `OutputFormat`
|gremlin.spark.persistContext |Whether to create a new context for every SparkGraphComputer or to reuse an available SparkContext in the JVM (or start one if not running)
