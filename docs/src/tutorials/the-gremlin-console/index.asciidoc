...
[llms-summary="A tutorial on using the Gremlin Console: the interactive REPL for working with graphs, running traversals, and connecting to remote servers.",allow-oversize="true"]
== The Gremlin Console
...
for all TinkerPop examples and test cases.  The full schema, origin, and example traversals for each graph are
documented in the link:https://tinkerpop.apache.org/docs/x.y.z/data/[TinkerPop Sample Data] reference.
...
link:https://tinkerpop.apache.org/docs/x.y.z/data/#modern[reference]).
...
link:https://tinkerpop.apache.org/docs/x.y.z/data/#the-crew[reference]).
...
link:https://tinkerpop.apache.org/docs/x.y.z/data/#grateful-dead[reference]).
...
[[result-iteration]]
As you have the Gremlin Console open you decide to debug the problem there. The examples that follow run against
the "modern" toy graph, so if `g` is not already available from the <<toy-graphs,toy graphs>> shown earlier,
create the graph and spawn a `g` from it first:
...
You copy your Gremlin code from
...
NOTE: While convenient, statically imported methods can be confusing for new users, especially when translating
code between the console (which is Groovy-based) and a Java IDE. The `in()` step deserves particular care, because
`in` overlaps with a reserved word in Groovy and a bare `in()` is not guaranteed to resolve to the intended
anonymous traversal step. The console will often resolve it, but the explicit `+__.in()+` form is recommended for
deterministic results and for parity with the equivalent Java code.
