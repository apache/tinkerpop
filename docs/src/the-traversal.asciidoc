...
[gremlin-groovy,modern]
----
g.V().branch {it.get().value('name') == 'marko' ? ['a'] : ['b']}.
       fork('a', __.values('age')).
       fork('b', __.values('name')) <1>
...
However, note that `choose()` can have an arbitrary number of forks.

[gremlin-groovy,modern]
----
g.V().has(label,'person').
       choose{ it.value('name') }.
         fork('marko', __.values('age')).
         fork('josh', __.values('name')).
         fork('vadas', __.valueMap()).
         fork('peter', __.label())
----
