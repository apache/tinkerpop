...
The `mapKeys()`-step (*flatMap*) takes an incoming map and emits its keys. This is especially useful when you're only interested in the top N elements in a `groupCount()` ranking.
...
g.V().hasLabel("song").out("sungBy").groupCount().by("name").mapValues().groupCount().
      order(local).by(valueDecr).limit(local, 3).mapValues() <3>
----
 
<1> Which artist sung how many songs?
<2> Get an anonymized set of song repertoire sizes.
<3> What are the 3 most common song repertoire sizes?
