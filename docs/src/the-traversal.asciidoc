...
An link:http://en.wikipedia.org/wiki/Infix_notation[infix notion] can be used as well. Though, with infix notation, only two traversals can be and'd together.
...
g.V().has(label, 'person').coalesce(values('nickname'), values('name'))
----
...
An link:http://en.wikipedia.org/wiki/Infix_notation[infix notion] can be used as well. Though, with infix notation, only two traversals can be or'd together.
