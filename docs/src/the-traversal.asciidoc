...
g.V().has(label,'person').
      choose({ it.value('name').length() == 5},   // If the predicate is true, then do `in`, else do `out`.
               __.in(), __.out()).values('name')
g.V().has(label,'person').
      choose({ it.value('name').length()},   // Use the result of the function as a key to the map of traversal choices.
             [ 5: __.in(),
...
gremlin> g.V().repeat(__.both()).times(3).profile().cap(TraversalMetrics.METRICS_KEY)
==>Traversal Metrics
                        Step                 Count  Traversers       Time (ms)    % Dur
           GraphStep(vertex)                     6           6           0.614     3.01
RepeatStep([VertexStep(BO...                    72          54          17.609    86.18
 SideEffectCapStep(~metrics)                     1           1           2.210    10.82
                       TOTAL                     -           -          20.434        -
----
