...
NOTE: To reduce the verbosity of the expression, it is good to `import static org.apache.tinkerpop.gremlin.process.traversal.dsl.graph.__.*`. This way, instead of doing `__.inE()` for an anonymous traversal, it is possible to simply write `inE()`. Be aware that `__.in()` and `__.as()` cannot take advantage of anonymous traversals because they are reserved keywords in Groovy.
