...
Shared with TinkerGraph code will be moved to AbstractTinkerGraph.
...
==== Transaction flows
===== Commit flow
1. make list of all affected elements by transaction#. The reason is to reduce the lock time.
2. if any has never version, then fail.
