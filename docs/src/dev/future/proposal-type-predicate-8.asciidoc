...
This enum itself will not be extensable, however it will contain a GlobalCache which providers can use to register their own types with the Grammar to be used as string arguments.
...
To reduce complexity of serialization and registration, providers who implement their own custom types will use `String` tokens only. They will need to register their type into the cache to be recognized by the embeded traversal and the Grammar.
