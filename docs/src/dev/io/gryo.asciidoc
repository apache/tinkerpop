...
As of TinkerPop 3.3.0, Gryo 3.0 was introduced and that is only partially compatible with 1.0. Attempts
to use 3.0 serializers with 1.0 serializers will likely lead to failure.

As of TinkerPop 3.4.0, Gryo 3.1 was introduced. It is compatible with 3.0 except for `VertexProperty` serialization.

All versions of Gryo support all the types defined by GraphSON as well as others that are bound more specifically
