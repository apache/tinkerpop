...
`Field.set()` on the provider's annotated class (see `AnnotatedTypeAdapter.fromFields` in
...
`fromFields` logic do not require reflective access and are unaffected by JPMS restrictions.
