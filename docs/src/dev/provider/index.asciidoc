...
request is saved on the server, unless it is too large to be held pending authentication, and a `AUTHENTICATE`
...
normally does without authentication.  A request that was not held, either because it was too large or because an
earlier failed authentication attempt already answered it, is not replayed and must be resent.  If it cannot
authenticate given the challenge response from the client, it will return `UNAUTHORIZED` (status code `401`).
