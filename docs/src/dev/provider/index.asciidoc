...
The simplest integration is to register `GqlDeclarativeMatchStrategy` in your graph's global strategy cache so that
every `GraphTraversalSource` created from your graph automatically supports `match()`. The pattern mirrors the way
TinkerGraph registers its own built-in strategies via a static initializer:
    ...
    static {
        TraversalStrategies.GlobalCache.registerStrategies(
                ...
                TraversalStrategies.GlobalCache.getStrategies(Graph.class).clone()
                        .addStrategies(GqlDeclarativeMatchStrategy.create(/* graph instance */)));  // <1>
    }
    // ...
}
----

<1> Because the global cache is shared across all instances of the graph class, the strategy must be able to locate the
    correct `Graph` instance at traversal execution time. The recommended approach is to create the strategy per
    `GraphTraversalSource` rather than statically, as shown below.

A cleaner per-instance approach is to override `traversal()` on your graph (or on your `GraphTraversalSource`
subclass) to inject the strategy automatically:

[source,java]
----
@Override
public GraphTraversalSource traversal() {
    return super.traversal().withStrategies(GqlDeclarativeMatchStrategy.create(this));  // <1>
}
----

<1> Passing `this` binds the strategy to the specific graph instance. The `DefaultGqlPlanner` and `DefaultGqlExecutor`
    created inside `GqlDeclarativeMatchStrategy.create()` are reused across all traversals from this source, so the
    query-graph parse cache is shared.

===== Performance Hints (Optional)

By default the TinkerGQL planner treats all vertex and edge label counts as unknown (`Long.MAX_VALUE`) and performs
full scans unless a property index is available. You can improve planning quality by overriding these methods on your
...
public long countVerticesByLabel(final String label) {
    // return the number of vertices with this label, or Long.MAX_VALUE if unknown
    return myLabelIndex.getCount(label);
}

@Override
public long countEdgesByLabel(final String label) {
    // return the number of edges with this label, or Long.MAX_VALUE if unknown
    return myEdgeLabelIndex.getCount(label);
}
----

These values guide the planner's join-ordering heuristic: extension steps backed by rarer labels are placed earlier
in the execution order, which prunes the DFS search space sooner. Approximate counts are acceptable; the planner
only uses them for relative comparison.

===== Property Index Integration (Optional)

If your graph maintains property indexes you can expose them to the TinkerGQL executor by returning a custom
`Graph.Index` from `Graph.index()`. When a `MATCH` pattern includes an inline property filter and the executor finds
an index for that key, it replaces the full vertex scan with a targeted index lookup:
...
public Graph.Index index() {
    return new Graph.Index() {
        ...
        public long countVertexIndex(final String key, final Object value) {
            // return Long.MAX_VALUE if 'key' is not indexed; otherwise the exact or approximate count
...
<2> Returning `Long.MAX_VALUE` for an un-indexed key signals the executor to fall back to a full vertex scan.
    Any value less than `Long.MAX_VALUE` — including `0` — means "the index was consulted and returned this count."
