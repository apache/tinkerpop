...
class corresponds to a given PDT name. Without this mapping, the driver will return a generic `CompositePDT`
object. To enable automatic round-trip hydration, providers must expose a pre-configured `PDTRegistry`
...
`PDTRegistry.create()`. Register them by adding a file at:
...
`[ProviderDefined]`-annotated types are discovered automatically. Calling `PDTRegistry.Create()`
...
Register hydration adapters explicitly on a `PDTRegistry` instance, then pass it to the connection:
...
pre-configured `PDTRegistry`. This shields end users from needing to know which types exist or how
...
public class MyGraphTypeRegistry {
    public static PDTRegistry create() {
        PDTRegistry registry = PDTRegistry.create(); // discovers ServiceLoader adapters
...
Register on the `PDTRegistry` in the same way as composite adapters.

===== Value Type

Across all GLVs, unhydrated primitive PDT values are represented as `PrimitivePDT` objects containing a
