...
implementation, such as `repeat()`, distributed OLAP steps, aggregation barriers, and strategy-based optimizations,
are deliberately excluded. The result is a small, self-contained, predictable execution model.

=== Supported Steps

Tiny Gremlin supports exactly the 33 steps listed below, grouped by category. No other step may be used in a Tiny
...
* `by()` modulation is not supported. Path objects always contain the raw elements visited.
