...
The `local` scope operates on list traversers (`Iterable`, `Iterator`, or array), converting each element. Non-list
...
Each value is converted to its canonical String representation. The list traverser forms
...
* If the incoming traverser isn't a list (array or Iterable) then an `Argument Error` is raised.
* If the argument doesn't resolve to a list (array or Iterable) then an `Argument Error` is raised.
...
* If the incoming traverser isn't a list (array or Iterable) or map then an `Argument Error` is raised.
* If the argument doesn't resolve to a list (array or Iterable) or map then an `Argument Error` is raised.
