...
`null` values return `null`, numbers evaluate to `true` if non-zero. Strings only accept "true" or "false" (case-insensitive), any other strings will return `null`.

*Exceptions*

If the incoming traverser type is unsupported or a string other than "true" or "false" then `null` will be returned.
