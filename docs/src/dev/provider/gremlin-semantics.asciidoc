...
*Syntax:* `E(Object... edgeIds)` | `E(List edgeIds)` | `E(Traversal idTraversal)`
...
* `edgeIds` - Zero or more edge identifiers. If none are provided, all edges are returned. A `List` may be provided
  as a single argument and will be expanded to look up each contained identifier.
...
*Description:* Filters traversers by property existence, property value, or label.

*Syntax:* `has(String key)` | `has(String key, Object value)` | `has(String key, P predicate)` | `has(String key, Traversal traversal)` | `has(T accessor, Object value)` | `has(T accessor, P predicate)` | `has(String label, String key, Object value)` | `has(String label, String key, P predicate)`

[width="100%",options="header"]
|=========================================================
|Start Step |Mid Step |Modulated |Domain |Range
|N |Y |N |`Element` / `Map` |`Element` / `Map`
|=========================================================

*Arguments:*

* `key` - The property key to check (or a `T` accessor for `id` or `label`).
* `value` - The value to compare against using equality. May be a literal or a `Traversal`.
...
* `has(key)` filters to elements that have the specified property (existence check). `hasNot(key)` is the inverse.
* `has(T.label, value)` is equivalent to `hasLabel(value)`. `has(T.id, value)` is equivalent to `hasId(value)`.
* When the domain is a `Map` (e.g., from `elementMap()` or `valueMap()`), `has()` filters by map key/value.
...
* For `within()`/`without()` predicates with traversal arguments, use `fold()` to collect multiple results into a list.
* `has(key, null)` filters to elements where the property value is `null` (not existence check).
...
*Syntax:* `hasId(Object id, Object... otherIds)` | `hasId(List ids)` | `hasId(P predicate)` | `hasId(Traversal traversal)`
...
* `id` - One or more identifiers to match against. A `List` may be provided as a single argument and will be expanded.
* `predicate` - A `P` predicate for identifier comparison. The predicate value may contain a `Traversal`.
* `traversal` - A child traversal whose first result is used as the identifier (implicitly wrapped in `P.eq()`).
  Must be read-only.
...
*Syntax:* `is(Object value)` | `is(P predicate)` | `is(Traversal traversal)`
...
* `value` - A literal value to test equality against.
* `predicate` - A `P` predicate for comparison. The predicate value may contain a `Traversal`.
* `traversal` - A child traversal whose first result is compared using equality (implicitly wrapped in `P.eq()`).
  Must be read-only.
...
* Operates on scalar values in the traversal stream (numbers, strings, etc.).
* When a `Traversal` is supplied as a value or inside a `P`, it is resolved per-traverser and its first result is used.
* Child traversals must be read-only; mutating steps are rejected with `IllegalArgumentException`.
...
*Description:* Adds or sets properties on elements. Returns the updated elements.
...
* `keyValues` - Additional key/value pairs and optional meta-properties.
* `cardinality` - The vertex property cardinality (`single`, `list`, `set`).
* `keyValues` (Map) - A `Map` of property key-value pairs to set.
...
* When following `addV()` or `addE()`, the step may be folded into the creation operation.
...
*Syntax:* `V(Object... vertexIds)` | `V(List vertexIds)` | `V(Traversal idTraversal)`
...
* `vertexIds` - Zero or more vertex identifiers. If none are provided, all vertices are returned. A `List` may be
  provided as a single argument and will be expanded to look up each contained identifier.
...
* `by()` - Specifies how to extract comparison values from the subject and/or the predicate target.

*Considerations:*

* `where(P)` with a scope-label value compares the current traverser (or labeled traverser) against the labeled value.
* `where(P)` with a traversal value resolves the child traversal per-traverser and compares directly.
