...
*Description:* Returns the difference between two Dates as a `Long`, in epoch seconds.
...
If the argument (or the `Traversal`) resolves to `null` it is treated as `0` (the epoch), so the result is the incoming
