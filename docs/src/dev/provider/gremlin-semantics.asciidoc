...
*Syntax:* `asString()` | `asString(local)`
...
* `scope` - Determines the type of travers it operates on. Both scope will operate on the level of individual traversers.
The `global` scope will operate on individual traverser. The `local` scope will operate on list traverser.

Null values from the incoming traverser are not processed and remain as null when returned.

*Exceptions*
* For `Scope.local`, if the incoming traverser is not a list of strings then an `IllegalArgumentException` will be thrown.
...
* `DT` - Date token enum. Supported values `second`, `minute`, `hour`, `day`.
* `Integer` - The number of units, specified by the DT Token, to add to the incoming values. May be negative for subtraction.
...
*Syntax:* `dateDiff(Date)` | `dateDiff(Traversal)`
...
* `Date` - Date for subtraction.
* `Traversal` - The `Traversal` value must resolve to a `Date`. The first result returned from the traversal will be
...
Format string. Variables can be represented with `%{variable_name}` notation.
...
* `String` - The string character(s) in the original string to be replaced. Nullable, a null input will be a no-op and
the original string will be returned
* `String` - The string character(s) to replace with. Nullable, a null input will be a no-op and the original string
will be returned
...
*Description:* Returns a reversed value of the incoming string or list traverser
...
|=========================================================
|Start Step |Mid Step |Modulated |Domain |Range
|N |Y |N |`String`/`array`/`Iterable` |`String`
|=========================================================
...
* `String` - The string character(s) used as delimiter to split the input string. Nullable, a null separator will split on
whitespaces.
