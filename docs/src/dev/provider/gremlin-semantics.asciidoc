...
*Syntax:* `E(Object... edgeIds)` | `E(Traversal idTraversal)`
...
* `edgeIds` - Zero or more edge identifiers. If none are provided, all edges are returned. Arrays and `Collection`
  values are flattened (unrolled) into individual IDs.
...
*Syntax:* `has(String key)` | `has(String key, Object value)` | `has(String key, P predicate)` | `has(String key, Traversal traversal)` | `has(T accessor, Object value)` | `has(T accessor, P predicate)` | `has(T accessor, Traversal traversal)` | `has(String label, String key, Object value)` | `has(String label, String key, P predicate)` | `has(String label, String key, Traversal traversal)`
...
* Child traversals must be read-only. Mutating steps are rejected with `IllegalArgumentException`.
...
* `IllegalArgumentException` if a child traversal contains mutating steps.
* `IllegalStateException` if the traverser is not an `Element` or `Property`.
...
*Syntax:* `hasId(Object id, Object... otherIds)` | `hasId(P predicate)`
...
* `id` - One or more identifiers, or Traversals supplying identifiers to match against. Arrays and
`Collection` values are flattened (unrolled) into individual IDs. Multiple IDs are combined into `P.within(...)`.
...
*Syntax:* `hasKey(String key, String... otherKeys)` | `hasKey(P predicate)` | `hasKey(Traversal traversal)`
...
*Syntax:* `hasLabel(String label, String... otherLabels)` | `hasLabel(P predicate)` | `hasLabel(Traversal traversal)`
...
*Syntax:* `hasValue(Object value, Object... otherValues)` | `hasValue(P predicate)` | `hasValue(Traversal traversal)`
...
*Syntax:* `is(Object value)` | `is(P predicate)`
...
*Syntax:* `property(Object key, Object value, Object... keyValues)` | `property(Cardinality cardinality, Object key, Object value, Object... keyValues)` | `property(Map keyValues)` | `property(Cardinality cardinality, Map keyValues)` | `property(Traversal mapTraversal)`
...
* Child traversals must be read-only; mutating steps are rejected with `IllegalArgumentException`.
* If a map traversal does not produce a `Map`, an `IllegalStateException` is thrown.
...
*Syntax:* `V(Object... vertexIds)` | `V(Traversal idTraversal)`
...
* `vertexIds` - Zero or more vertex identifiers. If none are provided, all vertices are returned. Arrays and
  `Collection` values are flattened (unrolled) into individual IDs.
...
*Syntax:* `where(P predicate)` | `where(String startKey, P predicate)` | `where(Traversal whereTraversal)`
...
* Child traversals inside predicates must be read-only; mutating steps are rejected with `IllegalArgumentException`.
