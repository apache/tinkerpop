...
to the corresponding resultTraversal and no further matches are attempted. If no match is found then the traverser is
filtered, but, for these cases, the `Pick.none` option can be provided to match those cases. If the choiceTraversal is
unproductive, then the traverser passes through by default or can be matched on `Pick.unproductive`.
...
`choose` does not allow more than one traversal to be assigned to a single `Pick`. The first `Pick` assigned via `option`
is the one that will be used, similar to how the first pickValue match that is found is used.
