...
string. TinkerPop does not provide a general purpose execution engine for this string and the user must consult their
graph database to determine what format is supported, if any. The second argument, when provided, supplies bound
parameters to the query; how (and whether) these are used is left to the provider.
...
* Each named variable in the matched pattern is stored as a labeled entry in the traverser's path, making
results retrievable by name via `select()`.
* Anonymous elements in the pattern (those without a variable name) are not exposed in the path and cannot be
referenced by `select()`.
* The traverser value itself after the step is unspecified by the framework; providers should set it to a
neutral sentinel (such as `Optional.empty()`) since callers are expected to use `select()` to retrieve results.
