...
* `edgeIds` - Zero or more edge identifiers. If none are provided, all edges are returned.
...
*Syntax:* `has(String key)` | `has(String key, Object value)` | `has(String key, P predicate)` | `has(String key, Traversal traversal)` | `has(String label, String key, Object value)` | `has(String label, String key, P predicate)`
...
* `traversal` - A child traversal whose first result is used as the comparison value (implicitly wrapped in `P.eq()`).
  Must be read-only.
* `label` - A label filter applied before the property check.
...
*Syntax:* `hasId(Object id, Object... otherIds)` | `hasId(P predicate)` | `hasId(Traversal traversal)`
...
* `id` - One or more identifiers to match against.
...
* `vertexIds` - Zero or more vertex identifiers. If none are provided, all vertices are returned.
