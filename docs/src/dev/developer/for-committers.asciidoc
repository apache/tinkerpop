...
* "the result should be a graph with _xxx_" - asserts a subgraph where the _xxx_ may be "vertices" or "edges" with a
follow-on table containing vertex or edge definitions respectively that are expected to exist in the subgraph
...
* `@RemoteOnly` - The scenario uses some Gremlin syntax that cannot be supported outside of remote test executions.
