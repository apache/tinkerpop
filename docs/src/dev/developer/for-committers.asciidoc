...
and carries its reasoning. A `record` points at an external artifact such as a JIRA issue or a
pull request, and is the only link between a bead and the code, because commit messages carry no
