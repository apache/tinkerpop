...
`tinkerpop-dev` skill is active, the committer opts in by telling the agent to use beads:
...
* *At session start* — invoke the `tinkerpop-dev` skill, then say "Use beads" to opt in.
...
Full workflow guidance for AI agents is in `.claude/skills/tinkerpop-dev/references/beads-workflow.md`.
