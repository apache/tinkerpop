...
`ONE_OR_MORE` or `ZERO_OR_MORE` vertex label cardinality — effectively, not single-label). Providers
that only support single-label vertices should exclude these tests.
* `@MultiLabelDefault` - The scenario simulates a provider whose default `valueMap()`/`elementMap()`
rendering returns labels as a set. The test infrastructure applies `with("multilabel")` to the
traversal source automatically (similar to how `@GraphComputerOnly` applies `withComputer()`).
Combined with `@MultiLabel` to require a multi-label graph.
...
* `@SingleLabelDefault` - The scenario simulates a provider whose default `valueMap()`/`elementMap()`
rendering returns a single label string. No additional `with()` configuration is applied by the test
infrastructure. Providers that override this behavior to return labels as a set by default should
exclude these tests.
...
IMPORTANT: 4.0.0-beta.2 Release - Removes websockets. This section will be updated as the new HTTP test server falls into place.
