...
new root is the right answer when it does not. A small fix is its own root rather than a child of
something larger. Declining to answer is the only response that does not work, because the agent
is then left with nowhere to record what it learns.
