...
`valueMap()`/`elementMap()`. The test infrastructure applies `with("multilabel")` to the traversal
source automatically (similar to how `@GraphComputerOnly` applies `withComputer()`). This simulates
a provider whose `valueMap()`/`elementMap()` returns labels as a set by default. Combined with
`@MultiLabel` to require a multi-label graph.
...
* `@SingleLabelDefault` - The scenario asserts that `valueMap()`/`elementMap()` returns a single
label string by default (without any `with()` configuration). Providers that override this behavior
to return labels as a set by default should exclude these tests.
