...
TinkerPop uses beads, a command line tool named `bd`, to capture that reasoning while it is still
exact. A bead can hold a decision that was taken, an alternative that was weighed and turned
...
durable project artifact. Beads remains entirely optional. No part of the build or the test suite
depends on it, and a contributor working without an agent has little reason to install it.

[[beads-setup]]
=== Setting Up Beads

The `bd` tool, available from link:https://beads.gascity.com/[the beads project], must be installed
before any of the commands below will work. The planning data is not held in Git. It lives in a
