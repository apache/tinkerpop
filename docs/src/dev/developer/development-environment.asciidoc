...
`NuGet.Config`, which the `dotnet` CLI cannot decrypt on Linux/macOS. Instead, set the key in the `NUGET_API_KEY`
environment variable before running `mvn deploy`, which the build passes explicitly to `dotnet nuget push --api-key`:
