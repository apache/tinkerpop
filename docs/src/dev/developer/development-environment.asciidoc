...
Docker based builds can be helpful to developers who do not want to setup a full Maven environment, which may be the
case for developers doing work on Gremlin Language Variants (e.g. Python). It allows them a way to do a full build of
the entire project to be sure that their changes are valid. Developers can also start Gremlin Server with its standard
test configuration:
