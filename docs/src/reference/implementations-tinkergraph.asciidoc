...
IMPORTANT: TinkerGraph itself stores labels as plain Java `String` values and places no restrictions on label content.
However, if a graph contains labels containing spaces, hyphens, or non-ASCII Unicode characters, those labels cannot
currently be referenced in a GQL `MATCH` query. This is a known limitation of the current grammar and may be addressed
in a future release.
...
cached in an LRU cache bounded to 1,000 entries). If a referenced parameter name is absent from the map at
execution time, the predicate evaluates to `false` and no vertices match that filter.
...
indexed property matches the filter value.
