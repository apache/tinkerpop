...
When the same variable appears in multiple comma-separated patterns, it acts as a join key where every element bound to
...
Variable names must be unique within a single element role, i.e., a name that refers to a node in one pattern cannot
...
are merged where the element must satisfy every predicate across all patterns in which the variable appears.
...
Multiple predicates in the same filter are ANDed where all predicates must hold for the element to match. All predicates
...
When a filter value is not known until execution time it may be supplied as a parameter reference (`$name`) in the query
string and the actual value passed in the params map using the `match(String, Map)` overload:
...
cached in an LRU cache bounded to 1,000 entries). The table below summarizes edge-case parameter behaviors:
...
constant but the parameter values change across calls. In this way, the query graph is parsed and cached once, and each
