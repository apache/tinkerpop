...
[[tinkergraph-gql]]
=== Declarative Pattern Matching (GQL)

TinkerGraph includes a built-in implementation of the `match(String)` step that accepts a subset of the
link:https://www.iso.org/standard/76120.html[ISO GQL] `MATCH` syntax. This allows declarative graph pattern queries
to be expressed directly against TinkerGraph. The implementation is intentionally minimal to help showcase how `match`
step can work.

Each result row is a `Map<String, Object>` keyed by the named variables in the pattern. `match()` can be used as a
terminal step to return all bindings directly, or followed by `select()` to project specific variables:
...
|`-[e:Label {key: 'value'}]->` |Named, labeled directed edge with an inline property filter — matches only edges where the property equals the literal value
|`-[e:Label {key: $param}]->` |Named, labeled directed edge with a parameter reference — value resolved from the params map at execution time
...
name are joined on that variable, which must resolve to the same element across all patterns. Perhaps, most notably,
there is not `RETURN` clause. `match` will always return the full binding `Map` and any projection must be performed
with Gremlin steps.
...
Note the use of `where` as a mechanism for filtering `match` results.

*Find people connected by a knows relationship in either direction using an undirected edge, binding the edge itself*
...
Any of the examples above could be written as a pure Gremlin traversal. The `match()` step becomes arguably preferable
when the query involves a multi-way join, where several paths must all connect through shared variables, because the
pattern expresses the shape of the subgraph directly rather than requiring the reader to trace variable bindings
through a chain of steps.

For example, the following Gremlin traversal using all imperative steps answers the question: "Find people who know
someone they also co-created software with":
...
and here is the same using `match`:
...
The three-way relationship where `a` created `s`, `b` created `s`, and `a` knows `b` is immediately visible as a pattern
in the `match()` form. In the Gremlin traversal, the reader has to follow the variable threading across `as()`,
`select()`, and `where()` to reconstruct the same topology.

IMPORTANT: TinkerGraph stores labels, variable names, and property keys as plain Java `String` values with no
restrictions on content. However, because the GQL grammar is identifier-based, any label, variable name, or property
key that contains spaces, hyphens, or non-ASCII Unicode characters cannot currently be referenced in a `MATCH` query.
For example, `MATCH (n:my-label)` and `MATCH (n {my-key: 1})` are not parseable; use simple alphanumeric identifiers
instead. This is a known limitation of the current grammar and may be addressed in a future release.

==== Variable Reuse and Multi-Pattern Joins

When the same variable appears in multiple comma-separated patterns it acts as a join key: every element bound to
that variable must be the same graph element across all patterns. This is the primary mechanism for expressing
multi-way joins in a single `MATCH` clause.

Variable names must be unique within a single element role — a name that refers to a node in one pattern cannot
refer to an edge in another pattern:
...
// Valid: same variable, same label — second occurrence is a bare reference that adds nothing
...
When a variable appears in a later pattern without a label (a bare reference such as `(n)` or `(n {age: 29})`),
any label already assigned to that variable from an earlier pattern is preserved. Predicates from all occurrences
are merged — the element must satisfy every predicate across all patterns in which the variable appears.

[source,text]
----
// 'n' already carries :Person from the first pattern; the bare (n {age: 29}) adds only the predicate
...
Both node and edge patterns can carry inline property filters. Filters are written as a comma-separated list of
`key: value` pairs inside curly braces immediately after the label (or variable, or the opening delimiter):
...
Multiple predicates in the same filter are ANDed: all predicates must hold for the element to match. All predicates
are equality tests; range or inequality tests are not yet part of the supported grammar.
...
|`42`       |Integer literal — defaults to `Integer` if the value fits, otherwise `Long` or `BigInteger`. Type suffixes: `29i` (Integer), `30l` (Long), `5b` (Byte), `1000s` (Short), `999n` (BigInteger).
...
*Find the co-creators of a specific piece of software using a node filter*
...
Property filters can appear on any node in the pattern, including intermediate and target nodes:

*Find software co-created with 'lop', filtering both the start and end vertices*

[gremlin-groovy,modern]
----
g.match("MATCH (a:person)-[:created]->(s:software {name: 'lop'}), (b:person)-[:created]->(s)").
  where("a", neq("b")).
  select("a", "b").by("name")
----

Edge patterns support the same filter syntax. The filter is applied to the edge itself before the target vertex is
resolved, so only edges whose properties satisfy the predicate contribute to the result.

The modern graph's `knows` edges carry a `weight` property, making it a natural example:

*Find high-weight knows relationships, binding the filtered edge*
...
When a filter value is not known until execution time — for example, because it comes from user input or an outer
traversal — supply it as a parameter reference (`$name`) in the query string and pass the actual value in the
params map using the `match(String, Map)` overload:
...
The same query string can be reused with different parameter maps without re-parsing (the parsed query graph is
cached in an LRU cache bounded to 1,000 entries). The table below summarises edge-case parameter behaviours:
...
When TinkerGraph has a vertex index on a filtered property key (created via `createIndex`), the executor
automatically uses the index for the seed vertex lookup, reducing the initial scan to only those vertices whose
indexed property matches the filter value. This is especially effective when the parameterized query pattern is
constant but the parameter values change across calls — the query graph is parsed and cached once, and each
invocation performs an index-backed seed lookup:
...
----
<1> Create the index once at graph setup time; all subsequent `match()` calls referencing `{name: $who}` use it automatically.

==== Unsupported Features

The current GQL engine supports a minimal subset of the ISO GQL `MATCH` syntax. The following features are
recognized in the specification but not yet implemented:

[width="100%",cols="2,4",options="header"]
|=========================================================
|Feature |Notes
|`WHERE` clause |Post-match filtering within the `MATCH` string is not supported. Use Gremlin's `where()` step after `match()` instead.
|`RETURN` clause |Projection within the `MATCH` string is not supported. Use `select()` after `match()` to project named variables.
|Inequality / range predicates |Only equality tests (`{key: value}`) are supported. Expressions such as `n.age > 30` or `n.name STARTS WITH 'A'` are not yet part of the grammar.
|Variable-length paths |Quantified path patterns such as `(a)-[:KNOWS*1..3]->(b)` are not supported.
|Multiple labels per element |`(n:Person:Employee)` — assigning more than one label to a node pattern — is not supported. Each node pattern accepts at most one label.
|`OPTIONAL MATCH` |Optional pattern matching (outer joins) is not supported.
|Aggregation functions |`COUNT`, `SUM`, `AVG`, and similar aggregation functions are not part of the supported grammar.
|=========================================================

In the meantime, equivalent behaviour for `WHERE` and `RETURN` can be expressed using the standard Gremlin step
vocabulary (`where()`, `select()`, `filter()`, `group()`, etc.).
