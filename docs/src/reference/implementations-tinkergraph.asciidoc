...
Node patterns can carry inline property filters that constrain which vertices are matched. Filters are written as a
comma-separated list of `key: value` pairs inside curly braces immediately after the label (or variable, or node
opening parenthesis):
...
Multiple predicates in the same filter are ANDed: all predicates must hold for a vertex to match. All predicates are
equality tests; range or inequality tests are not yet part of the supported grammar.
...
|`42`       |Integer literal (resolved as a `Long` value at match time)
|`3.14`     |Floating-point literal (resolved as a `Double` value at match time)
...
NOTE: Literal numeric types are fixed at parse time. An integer literal produces a `Long` and a float literal produces
a `Double`. If the stored property has a different numeric type (e.g. `Integer`), the equality test will not match
even if the numeric value is the same. Use a parameter reference and pass a value of the correct type in those cases.
...
cached internally). If a referenced parameter name is absent from the map at execution time, the predicate
evaluates to `false` and no vertices match that filter.
