...
`TinkerTransactionGraph`. TinkerGraph is deployed with TinkerPop and serves as the reference
...
<<tinkergraph-gremlin-persistence, persistence section>>. The in-memory `TinkerMemoryGraph` holds no data across JVM
restarts. Moving data in and out of any TinkerGraph in an interchange format is handled on demand by the `io()` step
