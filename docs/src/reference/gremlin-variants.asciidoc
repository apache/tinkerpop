...
// This book exercises only remote driver connections, but its predecessors leave Spark/Hadoop
...
[llms-summary="How Gremlin is embedded in host programming languages (Java, Groovy, Python, JavaScript, .NET, Go) and how language variants and drivers connect to a remote server."]
...
* Composite: `PDT("typeName",[key:"value",...])`  — creates a `CompositePDT` with a map of fields.
* Primitive: `PDT("typeName","value")` — creates a `PrimitivePDT` with an opaque string value.
...
[llms-summary="Using Gremlin from Go: installing gremlin-go, building traversals, Go syntax differences, and connecting to a remote Gremlin Server."]
[[gremlin-go]]
== Gremlin-Go
...
|ReadTimeoutMillis | Idle-read timeout in milliseconds. Bounds the wait for the initial server response and the idle time between response chunks, resetting per chunk. It is not a whole-request deadline. Set to `0` to disable. Also settable as `ReadTimeout` (a `time.Duration`); set only one. |0
...
Note that no driver timeout bounds the *total* duration of a request once it is under way. `ReadTimeout` bounds the
wait for the initial server response and the gap between response chunks, so a response that keeps producing chunks will not time out no matter how long it
...
The transaction-bound source retains its owning transaction. Calling `gtx.Tx()` returns the same `Transaction`, so
lifecycle operations can also be performed from the source with calls such as `gtx.Tx().Commit()`.
...
Cardinality functions that take a value are therefore referenced from `CardinalityValue` rather than from
`Cardinality` itself, and they follow the same Pascal-case convention as the step names described above:
`CardinalityValue.List(v)`, `CardinalityValue.Set(v)`, and `CardinalityValue.Single(v)`. Qualified with the
package name, these forms are `gremlingo.CardinalityValue.List(v)` and its `Set` and `Single` counterparts, and
each is supplied as a property value:
...
* GraphBinary permits map keys of any supported type, including composite values such as lists. Go's native `map` type
only supports comparable keys, so GraphBinary maps with non-comparable keys are not supported.
...
`*CompositePDT` structs containing a `Name` and `Fields` map without any configuration.
...
Working with raw `*CompositePDT` values is always available. Using a `PDTRegistry` is an optional
...
[llms-summary="Using Gremlin from Groovy: the JVM-based Gremlin-Groovy variant backed by Gremlin-Java, its role as the language of the Gremlin Console and Server, and its syntax differences."]
[[gremlin-groovy]]
== Gremlin-Groovy
...
[[gremlin-java]]
[llms-summary="Using Gremlin from Java: the reference implementation of the language, GraphTraversalSource construction, and connecting to a remote Gremlin Server."]
== Gremlin-Java
...
The following options are allowed on a per-request basis in this fashion: `batchSize`, `bulkResults`,
...
|connectionPool.readTimeoutMillis |Idle-read timeout in milliseconds. Bounds the wait for the initial server response and the idle time between response chunks, resetting per chunk. It is not a whole-request deadline. Set to `0` to disable. On `Cluster.Builder` also settable as `readTimeout(Duration)`. |0
...
IMPORTANT: 4.0.0-beta.3 Release - There is temporary support for GraphSON in the Java driver which will help with
...
The driver streams and deserializes a response incrementally as it arrives only when `GraphBinaryMessageSerializerV4`
is configured, which is the default. Any other `MessageSerializer`, such as a GraphSON serializer, causes the driver to
buffer the full response body before deserializing it.

[[gremlin-java-limitations]]
=== Limitations

* Java `Character` values do not support values outside the Basic Multilingual Plane, which are mainly 4-byte UTF-8
...
[[gremlin-java-dsl]]
[llms-summary="Creating a Domain Specific Language (DSL) in Java with the @GremlinDsl annotation from the gremlin-annotations module: defining the DSL interface, customizing the TraversalSource with additional start steps, and the traversal classes generated by the annotation processor."]
=== Domain Specific Languages
...
public interface SocialTraversalDsl<S, E> extends GraphTraversal.Admin<S, E> {
    ...
    public default <E2 extends Comparable> GraphTraversal<S, E2> youngestFriendsAge() {
...
* Increase the number of connections available in the connection pool by increasing the `maxConnections`.
...
[source,text]
----
Timed-out (150 MILLISECONDS) waiting for connection on Host{address=localhost/127.0.0.1:45940, hostUri=http://localhost:45940/gremlin}. Potential Cause: Number of active requests (1) exceeds pool size (1). Consider increasing the value for maxConnections.
...
Provider Defined Types (PDTs) allow graph providers to expose custom types through the driver. PDT values arrive as
`CompositePDT` objects containing a name and fields map without any configuration.
Consult your graph provider's documentation for the list of PDTs they support.
...
[source,java]
----
CompositePDT pdt = (CompositePDT) g.V().has("location").values("location").next();
String typeName = pdt.getName();                   // "x:Point"
Map<String, Object> fields = pdt.getFields();      // {x: 1.0, y: 2.0}
----

Working with raw `CompositePDT` objects is always available. The following two approaches are optional
...
[[gremlin-javascript]]
[llms-summary="Using Gremlin from JavaScript/Node.js: installing the gremlin package, building traversals, and connecting to a remote Gremlin Server."]
== Gremlin-JavaScript
...
* `Traversal.hasNext()`: Returns a `Promise` with a `Boolean` indicating whether a further result is available.
...
|options.readTimeoutMillis |Number |Idle-read timeout in milliseconds. Bounds the wait for the initial server response and the idle time between response chunks, resetting per chunk. It is not a whole-request deadline. Maps to undici `headersTimeout` and `bodyTimeout`. |runtime default
...
Note that no driver timeout bounds the *total* duration of a request once it is under way. `readTimeoutMillis` bounds the
wait for the initial server response and the gap between response chunks, so a response that keeps producing chunks will not time out no matter how long it
runs overall, and there is no client-side "overall" request timeout. If you need an absolute deadline, impose it in
...
IMPORTANT: 4.0.0-beta.3 Release - Event-based logging is not yet implemented for the HTTP driver. Listeners can be registered
...
The transaction-bound source retains its owning transaction. Calling `gtx.tx()` returns the same `Transaction`, so
lifecycle operations can also be performed from the source with calls such as `await gtx.tx().commit()`.
...
doubles. Typed wrappers (e.g. `toInt()`, `toDouble()`) can be used to control the exact type sent to the server. See
<<gremlin-javascript-numeric-types>>.
* GraphBinary `BigDecimal`, `Char`, and `Duration` values are not implemented.
* JavaScript `Date` values do not support the extreme values of GraphBinary's `DateTime` maximum and minimum. Those
...
Working with raw `CompositePDT` objects is always available. Using a `PDTRegistry` is an
...
[llms-summary="Using Gremlin from .NET: installing the Gremlin.Net package, its C# API with PascalCase step names, and connecting to a remote Gremlin Server."]
...
|ReadTimeoutMillis |Idle-read timeout in milliseconds. Bounds the wait for the initial server response and the idle time between response chunks, resetting per chunk. It is not a whole-request deadline. `0` (the default) disables it. Also settable as `ReadTimeout` (a `TimeSpan`; `Timeout.InfiniteTimeSpan` disables). |0
...
Note that no driver timeout bounds the *total* duration of a request once it is under way. `ReadTimeout` bounds
the wait for the initial server response and the gap between response chunks, so a response that keeps producing
...
|pdtRegistry |A `PDTRegistry` for hydrating and dehydrating <<gremlin-dotnet-pdt,Provider Defined Types>>. |`null`
...
The transaction-bound source retains its owning transaction. Calling `gtx.Tx()` returns the same `RemoteTransaction`,
so lifecycle operations can also be performed from the source with calls such as `await gtx.Tx().CommitAsync()`.
...
`GremlinClient.SubmitAsync<T>()` returns a `ResultSet<T>` that implements `IAsyncEnumerable<T>`, backed by a channel
that is filled as results are deserialized from the response. Results become available to the caller incrementally
as they are received. `ResultSet<T>` can be consumed directly with `await foreach`, or materialized into a
...
Because results are consumed incrementally, a server-side error (`ResponseException`) surfaces while the result
set is being iterated rather than when `SubmitAsync()` is called. Error handling should wrap the iteration or
...
The traversal API (`Next()`, `ToList()`, `HasNext()`, `ToSet()`, `Iterate()`) blocks internally on the underlying
...
Finally, the enum construct for `Cardinality` cannot have functions attached to it the way it can be done in Java,
therefore cardinality functions that take a value like `CardinalityValue.List(value)`, `CardinalityValue.Set(value)`,
and `CardinalityValue.Single(value)` are referenced from a `CardinalityValue` class rather than `Cardinality` itself.

[source,csharp]
----
g.AddV("person").Property("nicknames", CardinalityValue.List("okram"));
----
...
Several Gremlin types have a wider domain or finer precision than their closest C# counterparts. When the server
returns a value that falls outside the C# range, the outcome depends on the type. In most cases Gremlin.Net raises
an exception as it deserializes the response, so the error surfaces on the client at the point the result is read.
In a few cases the value is instead altered without any error. A character outside the Basic Multilingual Plane is
reconstructed incorrectly, and a fractional second finer than the C# tick is silently truncated.
...
Gremlin's `OffsetDateTime` permits offsets up to `+18:00`/`-18:00` and a much wider year range. When the offset
or year of the returned value lies outside the C# range, the `DateTimeOffset` constructor rejects it and
Gremlin.Net raises an `ArgumentOutOfRangeException` while reading the result. The offset bound is the more common
trigger, since a value such as `datetime('2018-03-22T00:35:44.741+18:00')` is a valid `OffsetDateTime` that
cannot be represented as a `DateTimeOffset`. A second offset restriction concerns granularity rather than
magnitude. `DateTimeOffset` records its UTC offset in whole minutes, whereas Java and GraphBinary express the
offset in seconds, so an offset that carries a sub-minute component cannot be represented and Gremlin.Net raises
an `ArgumentException` while reading the result. Precision is likewise a separate consideration from range.
Gremlin represents time with nanosecond precision, while `DateTimeOffset` measures time in 100-nanosecond ticks,
so a fractional second finer than one tick is silently truncated on deserialization without any error, and a
value such as `00.123456789` is read back as `00.1234567`.
* Gremlin's `Duration` type has a much larger range than C#'s `TimeSpan`, so a duration whose magnitude exceeds
`TimeSpan.MaxValue` or `TimeSpan.MinValue` cannot be constructed and Gremlin.Net raises an `OverflowException`
while reading the result. This affects only extreme values such as `Duration.FOREVER`. Durations within the
`TimeSpan` range are unaffected. As with `DateTimeOffset`, precision is a separate consideration from range.
Gremlin represents a duration with nanosecond precision, while `TimeSpan` measures time in 100-nanosecond ticks,
so a fractional-second component finer than one tick is silently truncated on deserialization without any error.
* Gremlin's `BigDecimal` supports up to 33 digits of precision while C#'s `decimal` type is limited to 28-29
significant digits. When the returned value exceeds the `decimal` range, the conversion overflows and
Gremlin.Net raises an `OverflowException` while reading the result.
* C#'s `char` holds a single UTF-16 code unit and cannot represent a code point outside the Basic Multilingual
Plane, which GraphBinary encodes as a four-byte `Char`. Such a value is decoded into a surrogate pair, of which
only the leading surrogate is returned, so the character is silently corrupted rather than raising an error.
Characters within the Basic Multilingual Plane are unaffected.
* C#'s `Dictionary` does not allow `null` keys, so a `Map` result that contains a `null` key cannot be populated
and Gremlin.Net raises an `IOException` while reading the result.
...
Provider Defined Types (PDTs) allow graph providers to expose custom types through the driver. PDT values arrive as
`CompositePDT` objects containing a `Name` and `Fields` dictionary without any configuration.
Consult your graph provider's documentation for the list of PDTs they support.
...
[source,csharp]
----
var pdt = (CompositePDT) g.V().Has("location").Values<object>("location").Next();
...
The `PDTRegistry.Create()` method scans loaded assemblies for `IProviderDefinedTypeAdapter<T>`
...
[[gremlin-python]]
[llms-summary="Using Gremlin from Python: installing gremlinpython, building traversals, Pythonic syntax differences, and connecting to a remote Gremlin Server."]
== Gremlin-Python
...
|read_timeout_millis |Idle-read timeout in milliseconds. Bounds the wait for the initial server response and the idle time between response chunks, resetting per chunk. It is not a whole-request deadline. Maps to aiohttp `sock_read`. Also settable as `read_timeout` (in seconds). |`None`
...
|pdt_registry |A `PDTRegistry` for hydrating and dehydrating <<gremlin-python-pdt,Provider Defined Types>>. |`None`
|=========================================================
...
Note that no driver timeout bounds the *total* duration of a request once it is under way. `read_timeout` bounds the
wait for the initial server response and the gap between response chunks, so a response that keeps producing chunks will not time out no matter how long it
runs overall, and there is no client-side "overall" request timeout. If you need an absolute deadline, impose it in
your application. `submit_async()` and `ResultSet.all()` return `concurrent.futures.Future` objects, so pass a
`timeout` to `result()`:
...
Provider Defined Types (PDTs) allow graph providers to expose custom types through the driver. PDT values arrive as
`CompositePDT` objects containing a `name` and `fields` dict without any configuration.
Consult your graph provider's documentation for the list of PDTs they support.
...
The `PDTRegistry.create()` class method discovers adapters via `entry_points` in `pyproject.toml`
