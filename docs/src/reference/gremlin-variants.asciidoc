...
Note that the `transport_factory` can allow for additional configuration of the `AiohttpTransport`, which exposes
options to manage `asyncio` timeouts, heartbeat, max content length, ssl options, and the ability to call the api from
an event loop:
