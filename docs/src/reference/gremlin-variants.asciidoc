...
Go rune literals are interpreted as `Integer` values rather than `Character` literals by default.
To use the Gremlin `Character` type, wrap the rune in `gremlingo.Char()`. For example:

[source,go]
----
g.Inject('a')                       // interpreted as integer 97 (the Unicode code point)
g.Inject(gremlingo.Char('a'))       // properly translated to a Gremlin character literal
----
...
* C# `char` values do not support values outside the Basic Multilingual Plane, which are mainly 4-byte UTF-8
characters. Those GraphBinary `Char` values are not supported.
