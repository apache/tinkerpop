...
* The `subgraph()`-step returns a detached `*Graph` data container, a read-only struct exposing
`Vertices map[interface{}]*Vertex` and `Edges map[interface{}]*Edge`. The result is not a live
`Graph` instance and cannot be passed to `traversal().with(...)`. To re-query subgraph elements against the
original graph, extract their `Id` and use `g.V(id)` / `g.E(id)` on the original `GraphTraversalSource`.
...
suffix (BigInteger).
* The `subgraph()`-step returns a detached `Graph` data container, a read-only object exposing
`vertices: Map<any, Vertex>` and `edges: Map<any, Edge>`. The result is not a live `Graph` instance and cannot be
passed to `traversal().with(...)`. To re-query subgraph elements against the original graph, extract their `id` and
use `g.V(id)` / `g.E(id)` on the original `GraphTraversalSource`.
...
instance and cannot be passed to `traversal().with(...)`. To re-query subgraph elements against the original graph,
extract their `Id` and use `g.V(id)` / `g.E(id)` on the original `GraphTraversalSource`.
...
* The `subgraph()`-step returns a detached `Graph` data container, a read-only object exposing `vertices: dict` and
`edges: dict` keyed by element id. The result is not a live `Graph` instance and cannot be passed to
