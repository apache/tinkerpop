...
* There is no default `set` type in Go. A set type code from the server is deserialized into a
`*gremlingo.SimpleSet`, which implements the `gremlingo.Set` interface, rather than into a slice. To read its
elements, the result should be type-asserted to `gremlingo.Set` and its `ToSlice()` method called. To input a set
into Gremlin-Go, a custom struct which implements the `gremlingo.Set` interface will be serialized as a set.
`gremlingo.NewSimpleSet` is a basic implementation of a set that is provided by Gremlin-Go that can be used to
fulfill the `gremlingo.Set` interface if desired.
...
* GraphBinary permits map keys of any supported type, including composite values such as lists. Go's native `map`
type only supports comparable keys, so GraphBinary maps with non-comparable keys are deserialized lossily rather than
rejected. A list key is converted to its string form through `fmt.Sprint`, so a key such as `[marko 29]` is stored as
the string `"[marko 29]"`, while a map key is stored under a pointer to the key value. In both cases the original
key's type and identity are lost, so such keys cannot be reconstructed or looked up by their original value.
