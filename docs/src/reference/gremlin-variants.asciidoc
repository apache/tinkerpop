...
|=========================================================
|Key |Description |Default
|ConnectTimeout |The TCP connect timeout (transport establishment, i.e. TCP connect plus TLS handshake where applicable, not an HTTP request timeout). |5 s
|IdleTimeout |How long idle connections stay in the pool before being closed. |180 s
|MaxConnections |The maximum concurrent connections to a single server. |128
|KeepAliveTime |Idle time before TCP keep-alive probes begin on an otherwise idle connection. Enables `SO_KEEPALIVE` on the socket and sets the per-socket idle time on Windows, Linux, and macOS; on other platforms keep-alive stays enabled at the OS default idle time. |30 s
|Compression |The response compression algorithm (`Compression.None` or `Compression.Deflate`). |`Compression.Deflate`
|DefaultBatchSize |The default batch size used to fill the per-request batch size when it is unset. |64
...
|ReadTimeout |The idle-read timeout applied to each individual read of the response stream. It resets per chunk. `Timeout.InfiniteTimeSpan` disables it. |`Timeout.InfiniteTimeSpan`
