...
When working with larger result sets it may be beneficial for memory management to process each chunk of data as it is returned from the gremlin server. The Gremlin JavaScript driver return a readable stream instead of waiting for the entire result set to be loaded.
...
readable.on('end', () => {
  console.log('query complete'); // when the finish event is received then all the results have been processed
})
----
