...
Several Gremlin types have a wider domain than their closest C# counterparts. When the server returns a value
that falls outside the C# range, the outcome depends on the type. In most cases Gremlin.Net raises an exception
as it deserializes the response, so the error surfaces on the client at the point the result is read. The one
exception is a character outside the Basic Multilingual Plane, which is reconstructed incorrectly without any
error.
...
cannot be represented as a `DateTimeOffset`.
