...
// This book exercises only remote driver connections, but its predecessors leave Neo4j/Spark/Hadoop
// plugins active. Exclude all three to restart the console with a clean classpath and avoid the
...
|responseSerializer.className |The fully qualified class name of the `MessageSerializer` that will be used to deserialize responses from the server. Note that the serializer configured on the client should be supported by the server configuration. |_none_
