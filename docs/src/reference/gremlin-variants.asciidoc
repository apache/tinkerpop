...
There are two ways to run Gremlin Server for these examples:

- *Running locally* – see the <<gremlin-server,Gremlin Server>> documentation.
- *Running with Docker*:
...
[source,shell]
----
cd gremlin-driver/src/main/java/examples
----
...
can be found in GitHub link:https://github.com/apache/tinkerpop/tree/x.y.z/gremlin-javascript/examples/node/[here]
and are designed to connect to a running <<gremlin-server,Gremlin Server>> configured with the
`conf/gremlin-server.yaml` and `conf/gremlin-server-modern.yaml` files as included with the standard release packaging.
...
[source,shell]
----
cd gremlin-javascript/examples/node
----
...
[source,shell]
----
cd gremlin-dotnet/Examples
----

For each example project, navigate to its folder, then build and run:
...
[source,shell]
----
cd gremlin-python/src/main/python/examples
----
