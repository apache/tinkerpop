...
JavaScript has a single `Number` type (IEEE 754 double) which cannot distinguish between JVM numeric types. The driver
...
are deserialized as `Number` and lose their original type information. Use `numberMode: 'precise'` to preserve the
