...
////
[[intro]]
[llms-summary="Introduction to TinkerPop and Gremlin: graph computing concepts (structure vs. process), the ways to connect to Gremlin (embedded, Gremlin Server, remote providers), and how to stay implementation-agnostic."]
= Introduction
...
1. They have a sense of what a graph is - not sure? see link:https://krlawrence.github.io/graph/#whygraph[Practical Gremlin - Why Graph?]
...
TinkerPop-enabled immediately expands the reach their system can get into different development ecosystems.
...
link:https://krlawrence.github.io/graph/["Practical Gremlin"] by Kelvin Lawrence. This book is
...
TIP: The link:https://tinkerpop.apache.org/docs/x.y.z/data/#modern[modern graph] and all of the other sample graphs
that TinkerPop provides are described in full, with their schemas and example traversals, in the
link:https://tinkerpop.apache.org/docs/x.y.z/data/[TinkerPop Sample Data] reference.
 ...
 * `Element`: maintains a collection of properties with the option for multiple labels to help classify the element type.
...
==>tinkermemorygraph[vertices:6 edges:6]
gremlin> g = traversal().with(graph)        // <2>
==>graphtraversalsource[tinkermemorygraph[vertices:6 edges:6], standard]
...
NOTE: The examples below connect to a Gremlin Server listening on `localhost` at port 8182, so a server must be
running at that address before a connection can succeed. The <<starting-gremlin-server,Starting Gremlin Server>>
section describes how to launch one with `bin/gremlin-server.sh conf/gremlin-server-modern.yaml`, which hosts the
...
link:https://krlawrence.github.io/graph/[Practical Gremlin] book for that sort of information.
...
object, but another graph like Amazon Neptune has an explicit subset of possible types. Choosing a type that is exotic
or perhaps is a custom type that only a specific graph supports might create migration friction should the need arise.
