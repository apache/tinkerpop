...
IMPORTANT: If using the `UnifiedChannelizer`, failures in evaluation will result in the session being closed and state
being lost. Asynchronous requests that are queued on the server will be cancelled and additional requests, in-flight or
otherwise will be rejected. Users should create a new session from the `Cluster` object in this case. The alternative,
to match the old `OpProcessor` GremlinServer behavior is to add a per-request option to `maintainStateAfterException`
and will instead have similar behavior to that described in this section.
