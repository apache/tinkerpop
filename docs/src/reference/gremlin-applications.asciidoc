...
Server's <<script-execution,sandbox>>. The `AllowListAuthorizer` denies the use of lambdas and OLAP operations in
bytecode requests and uses methods from the `BytecodeHelper` to detect these.
