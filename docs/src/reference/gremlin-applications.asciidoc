...
grpah these queries will be costly. If you are trying Gremlin MCP, please try it with a smaller subset of your graph for
...
Schema discovery uses Gremlin traversals and sampling:
...
The resulting schema helps assistants construct well‑formed traversals and explain results in natural language.
...
endpoint the server should use. Typical environment variables include:

* GREMLIN_ENDPOINT — host:port for the target Gremlin Server or compatible endpoint
* GREMLIN_USERNAME / GREMLIN_PASSWORD — credentials when authentication is enabled
* GREMLIN_USE_SSL — set to true when TLS is required by the endpoint
* LOG_LEVEL — optional logging verbosity for troubleshooting
