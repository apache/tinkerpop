...
<4> Note that there is a "tinkerpop.hadoop" plugin available, but it is not yet "active".
<5> Again, to use the "tinkerpop.hadoop" plugin, it must be made "active" with `:plugin use`.
<6> Now when the plugin list is displayed, the "tinkerpop.hadoop" plugin is displayed as "active".
