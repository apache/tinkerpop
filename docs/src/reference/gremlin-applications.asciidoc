...
* illegal removal of `Strategies`

During execution of a request, gremlin can prevent returning vertices or edges with specific labels or values of a
property, based on predicates specified in a `SubgraphStrategy'.

===== Mechanisms
Gremlin Server supports
three mechanisms to configure authorization:
...
link:https://tinkerpop.apache.org/docs/current/reference/#start-steps[start step],
one can apply so-called `Strategies` to these `GraphTraversalSources`, some of
which can serve for authorization purposes (`ReadOnlyStrategy`, `SubgraphStrategy`, `LambdaRestrictionStrategy`,
`EdgeLabelVerificationStrategy`), provided that users are not allowed to remove these `Strategies` afterwards. The
`ScriptFileGremlinPlugin` is found in the yaml configuration file for Gremlin Server:
+
[source,yaml]
----
scriptEngines: {
...
add the `Strategies` mentioned in item 1.
...
With implicit authorization (only adding restricting `Strategies` in the initialization script of Gremlin Server) all
authenticated users can access all hosted `GraphTraversalSources` and all face the same restrictions. One would need
separate Gremlin Server instances for each authorization policy and apply an authenticator that restricts access to a g
roup of users (that is, supports in authorization).
...
it by adding the following to Gremlin Server's yaml configuration file:
...
authentication: {
  ...
  config: {
    credentialsDb: conf/tinkergraph-credentials.properties}}
authorization: {
  authorizer: your.package.AllowListAuthorizer,
  config: {
    authorizationAllowList: your/path/allow-list.yaml}}
----

The `AllowListAuthorizer` does not implement the `Authorizer` interface directly, but rather extends the
`AbstractAuthorizer`. The latter class provides the logic to split user requests into gremlin bytecode requests and
groovy string requests. It also has the helper methods `runsLambda(Bytecode bytecode)` and
`runsVertexProgram(Bytecode bytecode)` to detect the presence of these operations in the gremlin bytecode.
...
import org.apache.tinkerpop.gremlin.server.authz.AbstractAuthorizer;
