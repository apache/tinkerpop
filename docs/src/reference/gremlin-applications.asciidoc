...
*It is not recommended to use older driver versions with 3.7.x Gremlin Server*, as some older drivers do not construct
...
default 3.7.x server, as well as if `ReferenceElementStrategy` is configured on the server (i.e. the expected behavior
from a 3.6.x server). This behavior can be observed with the results of `g.V().next()`.
