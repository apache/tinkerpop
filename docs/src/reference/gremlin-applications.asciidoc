...
remote execution. It is not a "full" implementation like `TinkerGraph` or `Neo4jGraph`, in that most of its methods
are not implemented. For example, calls to most methods like, `addVertex()` or `edges()` will result in an
`UnsupportedOperationException`. The only method used on `RemoteGraph` is `traversal()`, which generates a
`GraphTraversalSource` that includes a `RemoteStrategy`. The `RemoteStrategy` submits takes traversals constructed from
it, serializes them and submits them to Gremlin Server and the results are returned as though working with the `Graph`
