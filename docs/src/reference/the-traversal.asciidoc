...
NOTE: When using `limit()` inside <<repeat-step,`repeat()`>>, each repeat iteration maintains its own counter. 
...
NOTE: When using `range()` inside <<repeat-step,`repeat()`>>, each repeat iteration maintains its own counter. 
See the <<repeat-step,repeat step documentation>> for examples and details.
...
When using `limit()` or `range()` steps inside `repeat()`, each iteration of the repeat gets its own counter, ensuring 
...
Each repeat iteration maintains separate counters for `limit()` and `range()` steps, ensuring predictable behavior 
equivalent to manually unrolling the repeat.
