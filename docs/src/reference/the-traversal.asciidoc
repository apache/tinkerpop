...
NOTE: The `choose(P)` form does not support traversal-bearing predicates directly. To use a dynamic comparison as the
branch condition, wrap it in an `is()` step: `choose(__.is(P.op(traversal)), trueChoice, falseChoice)`.

[gremlin-groovy,modern]
----
g.V().values('age').choose(__.is(P.gt(__.V(1).values('age'))), __.constant('older'), __.constant('not older')) <1>
----

<1> For each age value, compare dynamically against marko's age using a traversal inside the predicate. If greater,
