...
`LabelCardinality` that supports mutation (e.g. `ZERO_OR_MORE` or `ONE_OR_MORE`). Adding a label that already exists
...
removes all labels. The graph must be configured with a `LabelCardinality` that supports mutation. Dropping a label
that does not exist on the element is a no-op.
...
emitted by `labels()` and is blocked by the `LabelsDropVerificationStrategy`. Use `dropLabel()` or `dropLabels()` to
remove labels from vertices.

See: <<addlabel-step,addLabel() step>>, <<labels-step,labels() step>>
...
See: <<label-step,label() step>>, <<addlabel-step,addLabel() step>>, <<droplabel-step,dropLabel() step>>
