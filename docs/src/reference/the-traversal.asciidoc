...
and executes it against the graph. The declarative query language string `match` accepts depends entirely on the
underlying graph database you are using. It is important to recognize that TinkerPop only defines the step signature
and result contract, but provides no general-purpose declarative query engine. *Whether this step is supported at all
depends on the graph system being used.* Consult the documentation for your graph system to determine what query
languages and syntax are accepted.
...
TinkerGraph, which includes a built-in implementation supporting a subset of GQL `MATCH` syntax (see
<<tinkergraph-gql>>), demonstrating how the step works in practice. What your graph system accepts may be entirely
different.
...
provider-specific. See <<tinkergraph-gql>> for the complete syntax supported by TinkerGraph's built-in GQL engine.
