...
Each named variable in the pattern is bound into the traverser's path history, making it available to subsequent
`select()` steps by name. Anonymous pattern elements, those without a variable names are not exposed and cannot be
selected.
