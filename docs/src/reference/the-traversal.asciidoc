...
`LabelCardinality` that supports mutation (e.g. `ZERO_OR_MORE` or `ONE_OR_MORE`).
...
removes all labels. The graph must be configured with a `LabelCardinality` that supports mutation.
