...
Provider Documentation. Tiny Gremlin is currently only supported in <<gremlin-javascript-tiny-gremlin,Javascript>>

=== Supported Steps

Tiny Gremlin supports the basic steps required to navigate, filter, mutate and extract values from elements in a graph.
These steps should provide the basics required to author simple traversals for data gathering, path finding and other
...
|Branching | <<repeat-step,`repeat()`>>, <<emit-step, `emit()`>>, <<times-step, `times()`>>, <<until-step, `until()`>>
|Ordering |<<order-step,`order()`>>
|Mutation |<<addvertex-step,`addV()`>>, <<addedge-step,`addE()`>>, <<property-step,`property()`>>
|Modulators |<<from-step,`from()`>>, <<to-step,`to()`>> - in conjunction with `addE()`, not `path()`.
|===
