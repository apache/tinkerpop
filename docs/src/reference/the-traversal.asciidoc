...
`true` when non-zero (and not `NaN`), `null` evaluates to `false` and strings are only accepted when
equal to `"true"` or `"false"` (case-insensitive). All other types will throw an `IllegalArgumentException`.
