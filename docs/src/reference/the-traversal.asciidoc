...
g.inject(1.76).asNumber(GType.INT) <3>
g.inject("1b").asNumber() <4>
g.inject(33550336).asNumber(GType.BYTE) <5>
----
...
the release of TinkerPop 3.4.0, Gremlin also supports text predicates (`TextP`), which are specialized predicates that
only work on String values and are of the form `Function<String,Boolean>`. Additionally, type predicate (`P.typeOf`)
supports filtering traversers based on their runtime types. The provided predicates are outlined in the table below and
are used in various steps such as <<has-step,`has()`>>-step, <<where-step,`where()`>>-step, <<is-step,`is()`>>-step, etc.
...
Gremlin provides type-based filtering capabilities through the `GType` enumeration, which represents the various data 
types that can be encountered during graph traversals. Each `GType` constant corresponds to a specific Gremlin type and
is primarily used with the `typeOf()` predicate to filter values based on their runtime type.
...
g.V().has("weight", P.typeOf(GType.DOUBLE))
----

The `NUMBER` type is particularly useful as it matches any numeric value, allowing for broader type-based filtering:
