...
NOTE: When using globally-scoped `limit()` inside <<repeat-step,`repeat()`>>, each repeat iteration maintains its own counter. 
...
NOTE: When using globally-scoped `range()` inside <<repeat-step,`repeat()`>>, each repeat iteration maintains its own counter. 
See the <<repeat-step,repeat step documentation>> for examples and details.
...
When using globally-scoped (default scope) `limit()`, `skip()`, or `range()` steps inside `repeat()`, each iteration of 
the repeat gets its own counter, ensuring consistent behavior across iterations:
...
<1> Starting from the song 'JAM' get 2 songs that have followed, repeating 3 times.
<2> Starting from the song 'DRUMS' get the 2nd and 3rd songs that have preceded, repeating twice.
<3> Starting from the song 'HEY BO DIDDLEY' get the songs that have followed, skipping the first 5 and repeating twice.

Each repeat iteration maintains separate counters, ensuring predictable behavior equivalent to manually unrolling the 
repeat.
...
NOTE: When using globally-scoped `skip()` inside <<repeat-step,`repeat()`>>, each repeat iteration maintains its own counter.
See the <<repeat-step,repeat step documentation>> for examples and details.
