...
[source,java,tab]
----
DetachStrategy detachStrategy = DetachStrategy.build().detachMode(DetachStrategy.DetachMode.CUSTOM).properties("name", "age").create();
// return Vertexes with only 2 properties, other properties are not included in the result
g.withStrategies(detachStrategy).V().iterate();
----
[source,groovy]
----
// return Vertexes with only 2 properties, other properties are not included in the result
g.withStrategies(new DetachStrategy(detachMode: "CUSTOM", properties: ["name", "age"])).V().iterate()
----
