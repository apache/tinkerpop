...
TinkerPop ships with a generous number of `TraversalStrategy` definitions, some of which are applied under the hood
and some of which can be applied explicitly by the user. Users can add `TraversalStrategy` definitions for particular
needs. The following sections detail how traversal strategies are applied and defined and describe a collection of
traversal strategies that are generally useful to end-users.

=== Application
A traversal can add or remove `TraversalStrategy` strategies with the `withStrategies()` and `withoutStrategies()`
<<start-steps, start steps>>, see the <<readonlystrategy, ReadOnlyStrategy>> and the <<barrier-step, barrier() step>>
as examples. One should keep the following in mind when modifiying a traversal's list of `TraversalStrategy` strategies:
...
try it nevertheless. As a result, the behavior in case of multiple application is unspecified. For some, like the
<<partitionstrategy, PartitionStrategy>> the first instance (currently) prevails. For others, like the
<<subgraphstrategy, SubgraphStrategy>>, the last instance (currently) prevails. The `OptionStrategy`
that is applied as a result of the `with()` steps is an exception in that it actually merges the parameters of
multiple `with()` steps.
