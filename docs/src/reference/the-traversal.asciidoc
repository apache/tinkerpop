...
whose values are the bound graph elements. This map is the current traverser value, so `match()` is valid as a
terminal step:
...
Named variables are also recorded in the traverser's path history, making them available to subsequent `select()`
steps by name. Anonymous pattern elements (those without a variable name) are not exposed and cannot be selected.
