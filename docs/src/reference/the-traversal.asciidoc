...
[gremlin-groovy,modern]
----
g.V(2).property(__.V(2).project('friendCount').by(__.both('knows').count())) <1>
g.V(2).valueMap()
----

<1> Set properties on vertex 2 from a Map produced by a child traversal. Here the `project()` step creates a Map with
