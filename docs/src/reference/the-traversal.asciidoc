...
pattern. This step was added to Gremlin as part of the TinkerPop 3.6 release. Prior to
that, it was necessary to write more complex queries using steps such as `coalesce`,
`fold` and `unfold`, or injecting maps into queries and writing additional steps to
manipulate them. The `mergeE` step makes these types of operation much simpler for the
query writer. The input passed to `mergeE` can be either a map, or a child traversal that
produces a map.

NOTE: There is a corresponding `mergeV` step that can be used when creating vertices.

Additionally, `optional` steps may be combined with `mergeE`, to take action depending on
...
source and target vertex IDs, as a parameter.  An ID and a label may also be provided but
this is optional. If the `T.id` or `T.label` are provided, they need to be enclosed in
parenthesis. The `mergeE` step can be used directly from the GraphTraversalSource - `g`,
...
One or more `option` steps can be used to control the behavior when an edge is created of
updated.
...
that, it was necessary to write more complex queries using steps such as `coalesce`,
`fold` and `unfold`, or injecting maps into queries and writing additional steps to
manipulate them. The `mergeV` step makes these types of operation much simpler for the
query writer. The input passed to `mergeV` can be either a map, or a child traversal that
produces a map.

NOTE: There is a corresponding `mergeE` step that can be used when creating edges.

Additionally, `optional` steps may be combined with `mergeV`, to take action depending on
whether a vertex was created, or already existed. There are various ways `mergeV` can
be used. The simplest being to provide a single map of keys and values as a parameter.  An
ID and a label may also be provided but this is optional. If the `T.id` or `T.label` are
provided, they need to be enclosed in parenthesis. The `mergeV` step can be used directly
...
<3> Create a vertex for Toby with an ID of 300.
...
A common scenario is to search for a vertex with a known ID and if it exists return that
vertex. If it does not exist, create it. As we have seen, one way to do this is to pass
the ID and all properties directly to `mergeV`. Another is to use `Merge.onCreate`. Note
that the map specified for `Match.onCreate` must include the ID if you want to create the
vertex with the ID originally searched for. The values provided to the `mergeV` map are
...
g.mergeV([(T.id): 1]).
    option(onCreate, fail("vertex did not exist")). <4>
    option(onMatch, [modified: 2022]) <5>

g.mergeV([(T.id):300]).
  option(Merge.onCreate,(Map)null)  <6>
----
...
<5> The vertex does exist, add a modified property.
<6> If the vertex does not exist, do nothing.
...
injecting a list of maps into the traversal and letting them stream into the `mergeV`
step.

[gremlin-groovy]
----
maps=
  [[(T.label) : 'Dog', name: 'Toby'  , breed: 'Golden Retriever'],
