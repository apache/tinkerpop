...
<11> Find all vertices whose age is greater than marko's age using a traversal inside a predicate. The child traversal
is evaluated per-traverser and its first result becomes the comparison value, consistent with `by(traversal)` semantics.
...
g.V().where(__.in('created').count().is(1)).values('name') <2>
g.V().where(__.in('created').count().is(gte(2))).values('name') <3>
g.V().where(__.in('created').values('age').
                           mean().is(inside(30d, 35d))).values('name') <4>
----

<1> Find all age values that are greater than marko's age using a traversal inside the predicate.
<2> Find projects having exactly one contributor.
<3> Find projects having two or more contributors.
<4> Find projects whose contributors average age is between 30 and 35.
