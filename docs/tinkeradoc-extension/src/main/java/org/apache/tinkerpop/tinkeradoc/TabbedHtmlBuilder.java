...
public class TabbedHtmlBuilder {
     ...
     * name to its reference section when one exists. Unknown tokens render as plain (unlinked) text.
     * The display name and anchor come from the shared {@link GraphCatalog}. The link uses the
     * {@code x.y.z} version placeholder, which {@code GremlinPostprocessor} replaces with the real
     * version in the final HTML (the same mechanism used by every other cross-reference in the docs).
     *
    ...
    static String graphNoteHtml(final String token) {
        ...
        final String name = (anchor != null)
                ? "<a href=\"https://tinkerpop.apache.org/docs/x.y.z/reference/#" + anchor + "\">"
    ...
    String build(final List<Tab> tabs, final String graphName) {
        ...
        // Graph-dataset caption: an italic line rendered just below the content box (as a child of
        // the tab section, after and cleared of the floated .tabcontent). It links to the toy
        // graph's reference section. Placed here so it never overlaps the code panel or the page.
