...
public class GremlinQueryParserTest {
    ...
    public void shouldParseVariables() {
        ...
        assertEquals(g.V().has("name", P.gt(50)).asAdmin().getGremlincode(),
                t.asAdmin().getGremlincode());
    }
    ...
    public void shouldParseVariablesInVarargs() {
        ...
        assertEquals(g.V(100).has("name", P.gt(50)).asAdmin().getGremlincode(),
                t.asAdmin().getGremlincode());

        t = (GraphTraversal<?, ?>) GremlinQueryParser.parse("g.V(x, y, 300).has('name',gt(z))", gremlinAntlrToJava);
        assertEquals(g.V(100, 200, 300).has("name", P.gt(50)).asAdmin().getGremlincode(),
                t.asAdmin().getGremlincode());
    }
    ...
    public void shouldParseVariableWithNoOp() {
        ...
        assertEquals(g.V().has("name", P.gt(null)).asAdmin().getGremlincode(),
                t.asAdmin().getGremlincode());
    }
    ...
    public void shouldParseMultiVariablesWithNoOp() {
        ...
        assertEquals(g.V(null, null, null).has("name", P.gt(null)).asAdmin().getGremlincode(),
                t.asAdmin().getGremlincode());
    }
}
