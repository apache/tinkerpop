...
public class HaltedTraverserStrategyConfigTest {

    @Parameterized.Parameter(0)
    public HaltedTraverserStrategy expectedStrategy;
    ...
    public void shouldRoundTripConfiguration() {
        ...
        final HaltedTraverserStrategy recreatedStrategy = HaltedTraverserStrategy.create(configuration);

        // Assert that the recreated strategy matches the expected strategy
        assertEquals(expectedStrategy.getHaltedTraverserFactory(), recreatedStrategy.getHaltedTraverserFactory());
    }
}