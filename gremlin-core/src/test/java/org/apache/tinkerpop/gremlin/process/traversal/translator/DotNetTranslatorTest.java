...
public class DotNetTranslatorTest {
    ...
    public void shouldTranslateValues() {
        final String script = translator.translate(g.V().values("name").asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().Values<object>(\"name\")", script);
    }

    @Test
    public void shouldTranslateValue() {
        final String script = translator.translate(g.V().properties().order().by(T.value, asc).value().asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().Properties<object>().Order().By(T.Value,Order.Asc).Value<object>()", script);
    }

    @Test
    public void shouldTranslateInject() {
        String script = translator.translate(g.inject(10,20,null,20,10,10).asAdmin().getGremlincode()).getScript();
        assertEquals("g.Inject<object>(10,20,null,20,10,10)", script);
        script = translator.translate(g.inject().asAdmin().getGremlincode()).getScript();
        assertEquals("g.Inject<object>()", script);
        script = translator.translate(g.inject((Object) null).asAdmin().getGremlincode()).getScript();
        assertEquals("g.Inject<object>(null)", script);
        script = translator.translate(g.inject(null, null).asAdmin().getGremlincode()).getScript();
        assertEquals("g.Inject<object>(null,null)", script);
        script = translator.translate(g.V().values("age").inject().asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().Values<object>(\"age\").Inject()", script);
        script = translator.translate(g.V().values("age").inject(null).asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().Values<object>(\"age\").Inject(null)", script);
        script = translator.translate(g.V().values("age").inject(null, null).asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().Values<object>(\"age\").Inject(null,null)", script);
    }

    @Test
    public void shouldTranslateGroup() {
        final String script = translator.translate(g.V().group("x").group().by("name").asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().Group(\"x\").Group<object,object>().By(\"name\")", script);
    }

    @Test
    public void shouldTranslateGroupCount() {
        final String script = translator.translate(g.V().groupCount("x").groupCount().by("name").asAdmin().getGremlincode()).getScript();
    ...
    public void shouldTranslateHasLabelNull() {
        String script = translator.translate(g.V().hasLabel(null).asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().HasLabel((string) null)", script);
        script = translator.translate(g.V().hasLabel(null, null).asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().HasLabel(null,null)", script);
        script = translator.translate(g.V().hasLabel(null, "person").asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().HasLabel(null,\"person\")", script);
        script = translator.translate(g.V().hasLabel(null, "person", null).asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().HasLabel(null,\"person\",null)", script);
        script = translator.translate(g.V().has(T.label, (Object) null).asAdmin().getGremlincode()).getScript();
    ...
    public void shouldTranslateHasNull() {
        String script = translator.translate(g.V().has("k", (Object) null).asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().Has(\"k\",(object) null)", script);
        script = translator.translate(g.V().has("l", "k", (Object) null).asAdmin().getGremlincode()).getScript();
        assertEquals("g.V().Has(\"l\",\"k\",(object) null)", script);
    }

    @Test
    public void shouldTranslateMergeVNull() {
        String script = translator.translate(g.mergeV((Map) null).option(Merge.onCreate, (Map) null).mergeV(__.identity()).asAdmin().getGremlincode()).getScript();
    ...
    public void shouldTranslateStringFunctionWithAndWithoutScopes(){
        assertEquals("g.Inject(1).AsString()",
                translator.translate(g.inject(1).asString().asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(1).AsString<object>(Scope.Local)",
                translator.translate(g.inject(1).asString(Scope.local).asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").Length()",
                translator.translate(g.inject("hello").length().asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").Length<object>(Scope.Local)",
                translator.translate(g.inject("hello").length(Scope.local).asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").LTrim()",
                translator.translate(g.inject("hello").lTrim().asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").LTrim<object>(Scope.Local)",
                translator.translate(g.inject("hello").lTrim(Scope.local).asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").RTrim()",
                translator.translate(g.inject("hello").rTrim().asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").RTrim<object>(Scope.Local)",
                translator.translate(g.inject("hello").rTrim(Scope.local).asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").ToUpper()",
                translator.translate(g.inject("hello").toUpper().asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").ToUpper<object>(Scope.Local)",
                translator.translate(g.inject("hello").toUpper(Scope.local).asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").ToLower()",
                translator.translate(g.inject("hello").toLower().asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").ToLower<object>(Scope.Local)",
                translator.translate(g.inject("hello").toLower(Scope.local).asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").Trim()",
                translator.translate(g.inject("hello").trim().asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").Trim<object>(Scope.Local)",
                translator.translate(g.inject("hello").trim(Scope.local).asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").Replace(\"o\",\"a\")",
                translator.translate(g.inject("hello").replace("o", "a").asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").Replace<object>(Scope.Local,\"o\",\"a\")",
                translator.translate(g.inject("hello").replace(Scope.local, "o", "a").asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").Split(\"o\")",
                translator.translate(g.inject("hello").split("o").asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").Split<object>(Scope.Local,\"o\")",
                translator.translate(g.inject("hello").split(Scope.local, "o").asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").Substring(1)",
                translator.translate(g.inject("hello").substring(1).asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").Substring<object>(Scope.Local,1)",
                translator.translate(g.inject("hello").substring(Scope.local, 1).asAdmin().getGremlincode()).getScript());

        assertEquals("g.Inject(\"hello\").Substring(1,2)",
                translator.translate(g.inject("hello").substring(1, 2).asAdmin().getGremlincode()).getScript());
        assertEquals("g.Inject(\"hello\").Substring<object>(Scope.Local,1,2)",
                translator.translate(g.inject("hello").substring(Scope.local, 1, 2).asAdmin().getGremlincode()).getScript());
    }
