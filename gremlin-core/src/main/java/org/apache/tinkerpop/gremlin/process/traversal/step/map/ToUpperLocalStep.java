 ...
 */
public final class ToUpperLocalStep<S, E> extends ScalarMapStep<S, E> {
        ...
        if (null == item) {
            ...
            while (iterator.hasNext()) {
                ...
                if (null == i) {
                    ...
                    resList.add(((String) i).toUpperCase());
                } else {
                    throw new IllegalArgumentException(
                        String.format("The toUpper(local) step can only take list of strings, encountered %s in list", i.getClass()));
                }
            }
            return (E) resList;
        } else if (item instanceof String) {
            return (E) ((String) item).toUpperCase();
        } else {
            // String values not wrapped in a list will not be processed in local scope
            throw new IllegalArgumentException(
                    String.format("The toUpper(local) step can only take string or list of strings, encountered %s", item.getClass()));
        }
