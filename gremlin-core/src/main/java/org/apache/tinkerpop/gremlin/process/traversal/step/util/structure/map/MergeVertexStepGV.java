...
public class MergeVertexStepGV<S> extends GValueStep<MergeVertexStep, S, Vertex> implements TraversalOptionParent<Merge, S, Map>, MergeElementStepStructure<S> {
    ...
    public MergeVertexStepGV(final Traversal.Admin traversal, final boolean isStart) {
        super(traversal, new MergeVertexStep(traversal, isStart));
    }

    public MergeVertexStepGV(final Traversal.Admin traversal, final boolean isStart, final Map merge) {
        super(traversal, new MergeVertexStep(traversal, isStart, merge));
    }

    public MergeVertexStepGV(final Traversal.Admin traversal, final boolean isStart, final GValue<Map> merge) {
        super(traversal, new MergeVertexStep(traversal, isStart, merge.get()));
        this.merge = new ConstantGVTraversal<>(merge);

    }

    public MergeVertexStepGV(final Traversal.Admin traversal, final boolean isStart, final Traversal.Admin<S,Map> mergeTraversal) {
