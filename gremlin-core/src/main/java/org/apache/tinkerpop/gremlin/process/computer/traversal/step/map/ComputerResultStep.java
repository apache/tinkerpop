...
public final class ComputerResultStep<S> extends AbstractStep<S, S> {
    ...
    public Traverser<S> processNextStart() {
        ...
        if (this.first && null == this.computerResult) {
            try {
                populateTraversers(this.graphComputer.program(TraversalVertexProgram.build().traversal(this.getTraversal(), !this.graphComputer.features().supportsDirectObjects()).create(this.graph)).submit().get());
