...
class TestGraphBinaryWriter(object):
    ...
    def test_offsetdatetime(self):
        ...
        output = self.graphbinary_reader.read_object(bytearray(b'\x88\x00\x00\x00\x07\xe6\x05\x14\x00\x00\x0b:s\xceZ\x00\x00\x00\x8c\xa0'))
        assert x == output

    def test_offsetdatetime_format(self):
        x = datetime.strptime('2022-05-20T03:25:45.678912Z', '%Y-%m-%dT%H:%M:%S.%f%z')
        output = self.graphbinary_reader.read_object(bytearray(b'\x88\x00\x00\x00\x07\xe6\x05\x14\x00\x00\x0b:s\xceZ\x00\x00\x00\x00\x00'))
        assert x == output

    def test_offsetdatetime_epoch(self):
        x = datetime.fromtimestamp(1690934400).astimezone(timezone.utc)
        output = self.graphbinary_reader.read_object(bytearray(b'\x88\x00\x00\x00\x07\xe7\x08\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'))
        assert x == output
