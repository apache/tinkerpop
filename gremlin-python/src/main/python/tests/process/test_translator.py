...
# These tests do not actually require that a server exists. As we
# never use a terminal step the actual connection will never be
# attempted. This will give us a 'g' object sufficient for our
# needs whether a server is present or not.
g, conn = connectToServer("localhost", 8182)
...
try:
    total += 1
    print(f'\n#{total-1}')
    print('Check constructor')
    tlr2 = Translator('g')
    assert tlr2.translate(tests[0][0].bytecode) == tests[0][1]
except:
    print(f'FAIL')
    failed += 1
else:
...
try:
    total += 1
    print(f'\n#{total-1}')
    print('Check querying source name')
    assert tlr2.get_traversal_source() == 'g'
except:
    print(f'FAIL')
    failed += 1
else:
