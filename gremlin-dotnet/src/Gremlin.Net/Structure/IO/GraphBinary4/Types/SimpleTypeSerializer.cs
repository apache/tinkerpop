...
namespace Gremlin.Net.Structure.IO.GraphBinary4.Types
{
    ...
    public abstract class SimpleTypeSerializer<T> : ITypeSerializer
    {
        ...
        /// <param name="valueFlag">The value flag byte that was read.</param>
        /// <param name="cancellationToken">The token to cancel the operation. The default value is None.</param>
        /// <returns>The read value.</returns>
        /// <remarks>
        /// Override this method to handle type-specific value flags (e.g., bulked format for List).
        /// The default implementation delegates to <see cref="ReadValueAsync(Stream, GraphBinaryReader, CancellationToken)"/>.
        /// </remarks>
        protected virtual Task<T> ReadValueAsync(Stream stream, GraphBinaryReader reader, byte valueFlag,
            CancellationToken cancellationToken = default)
        {
