...
* An issue's "status" should be in one of three states: `open`, `resolved` or `closed`.
** An `open` issue is newly created, under consideration or otherwise in progress.
** A `resolved` issue is completed for purposes of release (i.e. code, testing, and documentation complete).
** An issue is `closed` at time of release, meaning it has been delivered as a bug fix or improvement.
