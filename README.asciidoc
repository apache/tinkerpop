...
image:https://raw.githubusercontent.com/apache/tinkerpop/tinkerpop3/master/docs/static/images/tinkerpop3-splash.png[TinkerPop3]
...
** Specify specific tests in a TinkerPop Suite to run: `mvn test -Dgremlin.tests={test class 1, test class 2...}` - note that including a specific test may require its inner class implementation (e.g. `com.apache.tinkerpop.gremlin....UnionTest$ComputerTest`)
** Fast-build (does not run `neo4j-gremlin` tests - use with caution): `mvn clean install -DfeelingLucky`
** Clean the `.groovy/grapes/com.apache.tinkerpop` directory on build: `mvn clean install -DcleanGrapes`
