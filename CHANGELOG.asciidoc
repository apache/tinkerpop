...
* Added `Match(string)` and `Match(string, IDictionary)` overloads to `gremlin-dotnet` `GraphTraversal`, `GraphTraversalSource`, and `__` for declarative GQL pattern matching. Deprecated the traversal-based `Match(ITraversal[])` overloads in favor of the new string-based API.
