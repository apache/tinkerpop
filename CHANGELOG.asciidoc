...
* `ChooseStep` broken apart into `ChooseMapStep` and `ChooseBooleanStep` as the overloading was not clean.
* Added `TraversalStrategyRegistry` and `GraphTraversalStrategyRegistry` which has respective global strategies.
* Added a generic `BranchStep` to be used for "meta-step" re-writing for GraphComputer traversals.
