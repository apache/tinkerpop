...
* Added `TraversalLambda` which implements function, predicate, and consumer whose function is the traversal.
...
* Added `LazyBarrierStep` which acts like `BarrierStep` but operates on a seed with a bi-function.
