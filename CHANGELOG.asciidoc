...
* Renamed the `gremlin-driver` `serializer` configuration to `responseSerializer` to match the other GLVs and to make clear it only controls response deserialization (requests are always sent as JSON).
* Renamed the `gremlin-javascript` `reader` connection option to `responseSerializer` to match the other GLVs and to make clear it only controls response deserialization (requests are always sent as JSON).
