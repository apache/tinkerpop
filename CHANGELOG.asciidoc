...
* `Path` is now an interface with `DefaultImmutablePath` and `DefaultMutablePath` as implementations (2x speedup on path calculations).
...
* Add features for supporting the addition of properties via `Element.property(String,Object)`.
