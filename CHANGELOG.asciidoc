...
* Removed `Vertex` and `ReferenceVertex` from grammar. Use vertex id in traversals now instead.
* Changed `Element.properties` for Python and Javascript to always return an empty list when properties are not present as opposed to `None` and `undefined` respectively.
