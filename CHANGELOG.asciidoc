...
* Added a `Rune` type to `gremlin-go` to represent the Gremlin `Character` type in `GremlinLang` text generation and GraphBinary serialization, since Go's `rune` is only an alias for `int32` and cannot be distinguished from it.
