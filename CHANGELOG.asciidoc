...
* Hardened GraphBinary deserialization so that a malformed or crafted message can no longer trigger an oversized memory allocation.
* Added a `SerializationException` that is thrown when GraphBinary deserialization rejects a malformed message.
