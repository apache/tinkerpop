...
* Fixed up all cloning operations realizing that Java8 lambdas are always bound to the calling class (no delegates).
* Provided `local()` to signify the that previous sequence of steps is for a local traversal.
* Ensured that all the steps of a `Traversal` are cloned properly.
* Greatly increased the speed of `ScriptEngineLambda` by making use of static `ScriptEngine` cache.
