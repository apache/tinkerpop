...
* `SideEffectCapStep` now extends `SupplierBarrier` and is much simpler than before.
* Added `SupplierBarrier` which simply drains the traversal and emits the value of a provided supplier.
...
* `BarrierStep` is no longer abstract and added `GraphTraversal.barrier()` which is analogous to `fold().unfold()`, though cheaper.
...
* Added `ReducingBarrierStep` which acts like `BarrierStep` but operates on a seed with a bi-function.
...
* `LocalStep` traversal is treated as a branch, not an isolated traversal. Moreover, moved to `branch/`.
