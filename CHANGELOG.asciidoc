...
* Added `CloneableLambda', `ResettableLambda', and `TraversableLambda` interfaces to allow steps to process such lambdas as needed.
* Added `Traversal.Admin.getTraverserRequirements()` and removed `TraversalHelper.getTraverserRequirements(Traversal)`.
* Added `SmartLambda` which is smart about handling a `TraversalLambda` or a standard Java8 lambda.
...
* Added `__` static interface which allows for `__.out().out()`-style and `out().out()`-style construction of anonymous traversals (instead of `g.of()`).
