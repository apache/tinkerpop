...
* `PropertiesStep` and `PropertyMapStep` are the only property accessing steps that exist now. Simplification of the step library.
* `GraphTraversal.value(String)` no longer exists. Instead, use `GraphTraversal.
* `HiddenXXXStep` no longer exist. `PropertyXXXStep` and `ValueXXXStep` take a boolean to denote hidden accesss.
