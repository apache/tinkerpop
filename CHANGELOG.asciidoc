...
** Renamed the `Auth.BasicAuth`/`Auth.SigV4Auth` factory methods to `Auth.Basic`/`Auth.Sigv4`; the old methods are retained as `[Obsolete]` deprecated aliases that delegate to the canonical members, so existing code keeps compiling.
** Renamed `MaxConnectionsPerServer` to `MaxConnections`, `IdleConnectionTimeout` to `IdleTimeout`, and `KeepAliveInterval` to `KeepAliveTime` (now wired to a real TCP keep-alive socket option rather than the inert HTTP/2 ping timeout, enabling `SO_KEEPALIVE` and setting the per-socket idle time on Windows, Linux, and macOS, with a no-op fallback to the OS default idle time on other platforms); the old property names are retained as `[Obsolete]` deprecated aliases.
** Renamed `ConnectionTimeout` to `ConnectTimeout` (default lowered from 15s to 5s; old name retained as a deprecated alias). *(breaking)*
** Renamed `EnableCompression` to `Compression`, now a `{None, Deflate}` enum defaulting to `Deflate` (compression on by default; set `None` to disable). The deprecated `EnableCompression` `bool` is retained as a compatibility shim mapping `true`/`false` to `Compression.Deflate`/`Compression.None`. *(breaking)*
...
** Added `DefaultBatchSize` (default 64), a connection-level default that fills the per-request `batchSize` when unset.
