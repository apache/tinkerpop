...
* Greatly simplified the `InputFormat` and `OutputFormat` model for working with Giraph.
...
* Added `VertexProgram.getMessageScopes()` to allow vendors to know which `MessageScopes` are possible at a particular iteration.
