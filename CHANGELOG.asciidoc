...
* Added `propertyMap()` helper to `Vertex` in `gremlin-javascript`, `gremlin-python`, and `gremlin-dotnet` to view a vertex's properties as a map keyed by property key.
