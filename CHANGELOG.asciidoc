...
* `ComparatorSupplier<V>` interface returns a `List<Comparator<V>>` instead of a `Comparator<V>[]`.
* `T` now implements `Function<Element,Object>`.
* `ComparatorSupplier` and `ComparatorConsumer` interfaces now more elegant and vendor introspection support via `ElementValueComparator` and `ElementFunctionComparator`.
* Renamed `Comparing` marker interface to `ComparatorSupplier` to be consistent with Java8 naming conventions.
* `FunctionConsumer` interface provides vendor introspection via `ElementValueFunction`.
...
* Removed `Step.NO_OBJECT` as the problem is solves can be solved with proper use of `flatMap`.
