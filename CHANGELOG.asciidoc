...
* Prevent error in the Console when `:submit` is called but no remotes were configured.
* Provide a way to clean the `grapes` directory as part of a standard build with `mvn clean install`.
