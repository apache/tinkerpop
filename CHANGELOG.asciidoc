...
* Added the `T.labels` token as the token-level analogue of `labels()` and `Element.labels()`, projecting and filtering on the full vertex label set, and accepted by `mergeV()`, `addV()`, and `graph.addVertex()` as a synonym for `T.label`.
