...
* `Comparing<V>` interface returns a `List<Comparator<V>>` instead of a `Comparator<V>[]`.
* `T` now implements `Function<Element,Object>`.
* `Comparing` and `ComparatorAcceptor` interfaces now more elegant and vendor introspection support via `ElementValueComparator` and `ElementFunctionComparator`.
* `FunctionAcceptor` interface provides vendor introspection via `ElementValueFunction`.
